...
void do_racecommands( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_GOLEM(ch)) {do_golpowers(ch,argument);return;}
    ...
    {
		displayed = FALSE;
		if (ch->power[i] > 0 && strlen(discipline[i]) > 0 && (ch->level < 7 || argument[0] == '\0' 
		|| !str_prefix(argument, discipline[i])))
		{
			foundpower = 1;
			for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
			{
			    if (cmd_table[cmd].discipline == i && ch->power[i] >= cmd_table[cmd].disclevel)
				{
					if (!displayed)
					{
						displayed = TRUE;
	...
	if (cmd_table[cmd].race == 0)
		...
		if (cmd_table[cmd].race == ch->class && cmd_table[cmd].discipline ==0)
		{
		    strcpy(buf, "");
			if (i == 0)
		    {
		    ...
		    case CLASS_JEDI: sprintf(buf, " %15s : ", "Jedi Skills"); break;

		    default: sprintf(buf, " %15s : ", "Other Abilities");break;
				}
			}
			else if (i % 4 == 0)
			{
...
if ( 60 > ch->practice)
{
send_to_char("It costs 60 points of primal to create a Lesser
Klaive.\n\r",ch);
return;
}
ch->practice   -= 60;
...
const	struct	cmd_type	cmd_table	[] =
{
    ...
    /* Mage commands */
	{ "advocate",	do_advocate,	POS_STANDING,	 3,	 LOG_NORMAL, 0,0,0  },
    { "chant",		do_chant,		POS_FIGHTING,	 3, LOG_NORMAL, 0,0,0	},
    { "soulbond",       do_soulbond,            POS_STANDING,    3,  LOG_NORMAL, 0,0,0},
    { "solar",          do_solar,               POS_STANDING,    3,  LOG_NORMAL,0,0,0},
    { "necromancy",     do_necromancy,           POS_STANDING,   3,  LOG_NORMAL,0,0,0},
    { "inventory",	do_inventory,	POS_DEAD,	 	 0,  LOG_NORMAL, 0,0,0	},
    { "illusion",       do_illusion,            POS_STANDING,    3,  LOG_NORMAL,0,0,0},
    { "invocation",     do_invocation,          POS_STANDING,    3,  LOG_NORMAL,0,0,0},
    { "stop",           do_stop,                POS_FIGHTING,    3,  LOG_NORMAL, 0,0,0}, 
    { "planeshift",     do_planeshift,          POS_STANDING,    3,  LOG_NORMAL, 0,0,0},
    { "undeadcall",	do_undeadcall,	POS_STANDING,	3,	LOG_NORMAL, 0,0,0},
    { "airflesh",	do_airflesh,	POS_FIGHTING,	3,	LOG_NORMAL, 0,0,0},
    { "drainlife",	do_drainlife,	POS_FIGHTING,	3,	LOG_NORMAL, 0,0,0},
    { "ddoor",	do_ddoor,	POS_FIGHTING,	3,	LOG_NORMAL, 0,0,0},
    { "fade",	do_fade,	POS_STANDING,	3,	LOG_NORMAL, 0,0,0},
    { "pacify",	do_pacify,	POS_FIGHTING,	3,	LOG_NORMAL, 0,0,0},
    /* End new mage commands -Nuit */
    /* Mindflayer commands - Jace*/
    { "mindtalk",	do_mindtalk,	POS_DEAD,	1,	LOG_NORMAL,0,0,0},
    /* end Mindflayer commands */
    { "command",	do_command,		POS_SITTING,	 3,  LOG_NORMAL, 0,0,0	},
    ...
    { "version",	do_version,		POS_DEAD,	 	 7,  LOG_NORMAL, 0,0,0	},
    ...
    { "oldwho",		do_oldwho,			POS_DEAD, 0, LOG_NORMAL, 0,0,0 },
    { "who",		do_who,		POS_DEAD, 0,  LOG_NORMAL, 0,0,0 },
    { "wizlist",	do_wizlist,		POS_DEAD,	 	 0,  LOG_NORMAL, 0,0,0 },
    { "introduce",	do_introduce,	POS_STANDING,	 1,  LOG_NORMAL, 0,0,0 },
    ...
    { "clanship",	do_clanship,	POS_SITTING,	 2,  LOG_NORMAL},
    ...
    { "gsocial",	do_gsocial,		POS_DEAD,	 2, LOG_NORMAL, 0,0,0	},
    { "bounty",         do_bounty,              POS_DEAD,        0, 
LOG_NORMAL, 0,0,0       },  
    { "quest",		do_quest,		POS_SITTING,	 2,
LOG_NORMAL, 0,0,0	},
    ...
    { "llothsdestroy",	do_offer,		POS_SITTING,	 3,
LOG_NORMAL, 0,0,0	},
{ "worshipartemis", do_artemisworship, POS_SITTING, 3, LOG_NORMAL, 0,0,0 },
    ...
    { "sacrifice",       do_sacrifice,	POS_STANDING,    0,  LOG_NORMAL,
0,0,0	},
    /* Combat commands.*/
    { "autostance",	do_autostance,	POS_STANDING,	0, 0,0,0},
    { "generation", do_generation,	POS_STANDING,	10,	LOG_ALWAYS, 0,0,0 },
    { "class",      do_class,		POS_STANDING,	10,	LOG_ALWAYS, 0,0,0 },
    { "ptesttoggle",      do_ptesttoggle,	POS_STANDING, 0, LOG_ALWAYS, 0,0,0 },
    ...
    { "bash",		do_bash,		POS_FIGHTING,	 0, LOG_NORMAL, 0,0,0	},
    { "guardian",   do_guardian,	POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,2 },
    ...
    { "cull",		do_cull,		POS_STANDING,	3,	LOG_NORMAL, 0,0,0 },
    { "darkblaze",      do_darkblaze,     POS_STANDING,     0,   LOG_NORMAL, 0,0,0  },
    { "finger",		do_finger,		POS_SITTING,	 0,  LOG_NORMAL, 0,0,0	},
    { "checkpk",	do_checkpk,		POS_DEAD, 10,LOG_NORMAL},
    ...
    { "mclan",          do_mclan,               POS_STANDING,    3, LOG_NORMAL,CLASS_MONK,0,0},
    { "palmthrust",     do_palmthrust,   POS_FIGHTING,    3,LOG_NORMAL,CLASS_MONK,0,0},
    ...
    { "clone",      do_clone,		POS_STANDING,    	 3,  LOG_NORMAL, 8,DISC_VAMP_VICI,1 },
    { "tendrils",	do_tendrils,	POS_FIGHTING,      3,  LOG_NORMAL, 8,DISC_VAMP_OBTE,6 },
    ...
    { "zombie",     do_zombie,		POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,3 },
    { "wall",       do_wall,		POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,7 },
    { "facade",     do_facade,		POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_PRES,6 },
    { "baal",       do_baal,		POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,9 },
    { "dragon",     do_dragon,		POS_SITTING ,      3,  LOG_NORMAL, 8,DISC_VAMP_VICI,8 },
    { "dragonform", do_dragonform,	POS_STANDING,    3,  LOG_NORMAL, 8,DISC_VAMP_VICI,7 },
    { "spew",       do_spew,		POS_FIGHTING,      3,  LOG_NORMAL, 8,DISC_VAMP_THAU,6 },
    { "bloodwater", do_bloodwater,	POS_FIGHTING,    3,  LOG_NORMAL, 8,DISC_VAMP_THAU,9 },
	{ "strangle",	do_throttle,	POS_FIGHTING,		3,	LOG_NORMAL,	0,0,0 },
    ...
    { "dub",        do_dub,			POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_OBFU,4 },
    ...
    { "diablerize",	do_testemb,		POS_STANDING,      3,  LOG_NORMAL, 8,0,0},
    { "testarm",	do_testarm,		POS_STANDING,		11,  LOG_NORMAL, 0,0,0 },  
	{ "knee",		do_knee,		POS_FIGHTING,		3,	LOG_NORMAL, 0,0,0 },
    { "kneel",		do_worship,		POS_MEDITATING,		0,  LOG_NORMAL, 0,0,0	},
    { "powers",		do_racecommands,POS_FIGHTING,		3, LOG_NORMAL,0,0,0 },
	{ "ceremony",	do_ceremony,	POS_STANDING,		3,	LOG_NORMAL, 0,0,0 },
    { "taste",      do_taste,		POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_THAU,1 },
    { "gate",       do_gate,		POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,1 },
    ...
    { "decay",      do_decay,		POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,5 },
    ...
    { "for",		do_for,			POS_DEAD,		 11, LOG_NORMAL, 0,0,0 },
	 /* Monk */
    { "decide",		do_decide,	POS_STANDING,	3,	LOG_NORMAL, 0,0,0},	 
    { "expel",		do_expel,	POS_STANDING,	3,	LOG_NORMAL, 0,0,0},	     
    { "mantra",		do_mantra,	POS_STANDING,	 3,  LOG_NORMAL, 64,0,0 },
    { "guide",		do_guide,	POS_STANDING,	 3,  LOG_NORMAL, 64,0,0 },
	{ "mastery",	do_mastery, POS_STANDING,	 3, LOG_NORMAL, 64,0,0},
    { "cloak",		do_cloak,	POS_STANDING,      3,  LOG_NORMAL, 64,0,0 },
    { "pray",		do_pray,	POS_RESTING,	 1,  LOG_NORMAL, CLASS_DEMON,0,0 },
...
{ "llothsbless", do_llothsbless, POS_FIGHTING, 3, LOG_NORMAL, 64,0,0 },
    { "godsbless",	do_godsbless,POS_FIGHTING,	 3,  LOG_NORMAL, 64,0,0 },
	{ "monktalk",       do_monktalk,   POS_DEAD,       1, LOG_NORMAL, 64,0,0 },      
    { "disciplines",    do_disciplines,POS_DEAD,   3,  LOG_NORMAL, 0,0,0 },
    { "research",		do_research,POS_STANDING,      3,  LOG_NORMAL, 0,0,0 },
	{ "monkclan",	do_monkclan, POS_STANDING,	 3, LOG_NORMAL, 64,0,0},
    /* Vampire and werewolf commands.   */
    { "bloodwall",	do_bloodwall,POS_MEDITATING,	 3,  LOG_NORMAL, 8,DISC_VAMP_THAU,7 },
    { "bloodrod",		do_bloodrod,POS_STANDING,      3, LOG_NORMAL, 8,DISC_VAMP_THAU,2 },
    ...
    { "ritual",		do_wolfritual, POS_STANDING,   3, LOG_NORMAL, 4,0,0 },
    ...
    { "form",		do_form,       POS_DEAD,      7, LOG_ALWAYS, 0,0,0},
    ...
    { "claws",	do_claws,	POS_FIGHTING,	3,	LOG_NORMAL,1,NPOWER_NINGENNO,1},
    { "fangs",		do_fangs,	POS_FIGHTING,  3,  LOG_NORMAL, 1,DISC_DAEM_ATTA,2 },
    { "horns",		do_horns,	POS_FIGHTING,  3,  LOG_NORMAL, 1,DISC_DAEM_ATTA,4 },
    { "stigmata",	do_stigmata,	POS_FIGHTING,  3,  LOG_NORMAL, 1, DISC_DAEM_ATTA,7},
    ...
    { "transfer",       do_transfer,    POS_DEAD,      7,  LOG_NORMAL, 
0,0,0 },
...
0,0,0 },  
    ...
    { "addlag",         do_addlag, POS_DEAD,              11,
LOG_ALWAYS, 0,0,0,}, 
    { "bane",		do_bane,	POS_FIGHTING,  3,  LOG_NORMAL,
1,DISC_DAEM_CORR, 3 },
    ...
    { "stealsoul",	do_stealsoul,   POS_STANDING,  3,
LOG_NORMAL,1,0,0 },
	{ "beastdown",	do_beastdown,	POS_STANDING,  3,  LOG_NORMAL, 1,0,0 },
	{ "daywalker",	do_daywalker,	POS_STANDING,  3,  LOG_NORMAL, 1,0,0 },
	{ "champions",  do_champions,	POS_DEAD,  3,  LOG_NORMAL, 1,0,0 },
    ...
    { "cauldron",		do_cauldron,POS_FIGHTING,      3,  LOG_NORMAL, 8,DISC_VAMP_THAU,5 },
    { "flamehands",	do_flamehands,POS_FIGHTING,	 3,  LOG_NORMAL, 8,DISC_VAMP_PROT,5 },
    { "drain",		do_drain,	POS_FIGHTING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,4 },
    { "summon",		do_summon,	POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,1 },
    { "shade",		do_shade,	POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_OBTE,1 },
    ...
    { "satan",          do_satan,   POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_DAIM,10 },
    { "rot",            do_rot,     POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_VICI,6 },
    ...
    { "acid",           do_acid,    POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_VICI,9 },
    { "death",          do_death,   POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_QUIE,5 },
    { "flash",          do_flash,   POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_QUIE,9 },
    { "tide",   		do_tide,    POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_THAU,10 },
    ...
    { "anarch",		do_anarch,	POS_STANDING,	 3,  LOG_NORMAL, 8,0,0	},
    ...
    /*{ "outcast",		do_outcast,	POS_STANDING,	 3,  LOG_ALWAYS, 0,0,0 },*/
    { "darkside",                do_darkside,     POS_STANDING,    0,  LOG_NORMAL, 0,0,0  },
    { "shadowstep",	do_shadowstep,POS_STANDING,    3,  LOG_NORMAL, 8,DISC_VAMP_OBTE,7 },
    ...
    { "favour",		do_favour,	POS_SITTING,	 3,  LOG_NORMAL, 8,0,0	},
    ...
    { "theft",          do_theft,   POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_THAU,4 },   
    { "plasma",         do_plasma,  POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_VICI,5 },
    { "zuloform",       do_zuloform,POS_FIGHTING,      3,  LOG_NORMAL, 8,DISC_VAMP_VICI,4},
    { "beckon",         do_beckon,  POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_ANIM,1},
    { "scales",		do_scales,	POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_SERP,6 },
    { "cserpent",       do_cserpent,POS_STANDING,      3,  LOG_NORMAL, 8,DISC_VAMP_SERP,5 },
/* Mecha Shit*/
{ "kamikazi", do_kamikazi, POS_FIGHTING, 3, LOG_NORMAL, 0, 0, 0},
{ "repair", do_mechaheal, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},
{ "clusterbomb", do_cluster_bomb, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},
{ "mechatalent", do_mechatalent, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},
{ "pscionicscream", do_pscionic_scream, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "academy", do_academy, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "nightvision", do_infro, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "lowblow", do_lowblow, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "mechastats", do_mechastats, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "concussiongrenade", do_concussion_grenade, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "engineer", do_engineer, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "flight", do_flight, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},   
{ "commstat", do_commstat, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},
{ "jumpjets", do_jump_jets, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},
{ "burst", do_burst, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},
{ "mecharush", do_mecha_rush, POS_STANDING, 3, LOG_NORMAL, 0, 0, 0},
{ "psychicblast", do_psychicblast, POS_FIGHTING, 3, LOG_NORMAL, 0, 0, 0},
/*End of Mecha Shit*/
{ "set",	do_set,	POS_RESTING,	2, LOG_ALWAYS, 0, 0, 0},
/*start ninja section*/
	
	{ "tsukuru", do_ninjacreate,	3,	LOG_NORMAL, 0,0,0 },
    { "principles",     do_principles,POS_STANDING,    3,  LOG_NORMAL, 0,0,0},
    { "michi",          do_rage,   POS_STANDING,      3,  LOG_NORMAL, 0,0,0},
	{ "inoru",		do_calm, POS_STANDING,	3, LOG_NORMAL, 0,0,0 },
    { "ninjainduct",      do_discipline,POS_STANDING,    3,  LOG_NORMAL, 0,0,0},
    { "kakusu",         do_kakusu,  POS_STANDING,      3,  LOG_NORMAL, 0,0,0},
    { "kanzuite",       do_kanzuite,POS_STANDING,      3,  LOG_NORMAL, 0,0,0},
    { "mienaku",        do_mienaku, POS_FIGHTING,      3,  LOG_NORMAL, 0,0,0},
    { "target",         do_target,  POS_STANDING,      3,  LOG_NORMAL, 0,0,0},
    { "nclan",          do_nclan,   POS_STANDING,      3,  LOG_NORMAL, 0,0,0},
    { "bomuzite",       do_bomuzite,POS_STANDING,      3,  LOG_NORMAL, 0,0,0},
    { "mitsukeru",      do_scry, POS_STANDING,         3,  LOG_NORMAL, 0,0,0},
    { "project",		do_project,	POS_DEAD,	 1, LOG_NORMAL, CLASS_NINJA,0,0 },
    { "koryou",		do_readaura,POS_STANDING,	 3,  LOG_NORMAL, 0,0,0	},
    { "dokomo",		do_dokomo,POS_STANDING,	 3,  LOG_NORMAL, 0,0,0	},    
/*start drow section */
    { "circle",		do_circle,	POS_FIGHTING,	 2,  LOG_NORMAL, 0,0,0 },
    { "sign",		do_sign,	POS_FIGHTING,	 2,  LOG_NORMAL, 0,0,0 },
    { "relevel",		do_relevel,	POS_DEAD,   	 0,  LOG_ALWAYS, 0,0,0 },
    { "supplicate",		do_supplicate, POS_STANDING,   3, LOG_NORMAL, 0,0,0 },
    { "favor",		do_favor,	POS_STANDING,      3,  LOG_NORMAL, 0,0,0 },
    { "grant",		do_grant,	POS_STANDING,	 3,  LOG_NORMAL, 0,0,0 },
    { "drows",		do_drows,	POS_FIGHTING,      2,  LOG_NORMAL, 0,0,0 },
    ...
    { "mask",		do_mask,	POS_STANDING,	 3,  LOG_NORMAL, 8,DISC_VAMP_OBFU,1	},
    { "mortal",		do_mortal,	POS_FIGHTING,	 3,  LOG_NORMAL, 8,DISC_VAMP_OBFU,2	},
    ...
    { "scry",		do_scry,	POS_STANDING,	 3,  LOG_NORMAL, 8,DISC_VAMP_AUSP,2	},
    ...
    { "shield",		do_shield,	POS_SITTING,	 3,  LOG_NORMAL, 8,DISC_VAMP_OBFU,3	},
    { "teach",		do_teach,	POS_STANDING,	 3,  LOG_NORMAL, 0,0,0	},
    { "totems",		do_totems,	POS_SITTING,	 3,  LOG_NORMAL, 4,0,0	},
    { "tribe",		do_tribe,	POS_SITTING,	 3,  LOG_NORMAL, 4,0,0	},
    { "truesight",	do_truesight,POS_SITTING,	 3,  LOG_NORMAL, 8,DISC_VAMP_AUSP,1	},
    { "upkeep",		do_upkeep,	POS_DEAD,	       3,  LOG_NORMAL, 0,0,0	},
    { "vanish",		do_vanish,	POS_STANDING,	 3,  LOG_NORMAL, 4,DISC_WERE_OWL,1	},
    { "vclan",		do_vclan,	POS_DEAD,	 	 3,  LOG_NORMAL, 8,0,0	},
    ...
    /* Immortal commands.    */
    { "laymortal", do_makepreg, POS_DEAD, 10, LOG_ALWAYS},
    { "curseditem",	do_curseditem,	POS_DEAD, 7, LOG_NORMAL},
    { "eatmortal",	do_eatmortal,	POS_DEAD, 10, LOG_NORMAL},
    { "pdelete",	do_pdelete,	POS_DEAD, 12, LOG_ALWAYS},
    { "unrestore",	do_unrestore,	POS_DEAD, 9, LOG_NORMAL},
    { "beep",		do_beep,	POS_DEAD, 9, LOG_NORMAL},
    { "watch",		do_watch,	POS_DEAD, 10,LOG_NORMAL},
    { "immspar",	do_immspar,	POS_DEAD, 12,LOG_NORMAL},
    { "rename",		do_rename,	POS_DEAD, 11,LOG_NORMAL},
    { "multiple",	do_multiple,	POS_DEAD, 8, LOG_NORMAL},
    { "randomload",	do_randomload,	POS_DEAD, 10,LOG_NORMAL},
    { "classconfig",	do_classconfig,	POS_DEAD, 11,LOG_NORMAL},
    { "classme",	do_classme,	POS_STANDING,2,LOG_NORMAL},
    { "loaddummy",	do_loaddummy,	POS_DEAD, 11,LOG_NORMAL},
    { "setimmpower",	do_setimmpower,	POS_DEAD, 12,LOG_ALWAYS},
    { "settitle",	do_settitle,	POS_DEAD, 10,LOG_NEVER},
    { "freevnums",	do_freevnums,	POS_DEAD, 8, LOG_NORMAL},
    { "daynight",	do_daynight,	POS_DEAD, 9, LOG_NORMAL},
    { "bugfind",	do_findbug,	POS_DEAD,		0, LOG_ALWAYS, 0, 0, 0},
    ...
    { "clearreply",	do_clearreply,	POS_DEAD, 0, LOG_NORMAL},
    { "clearpks",	do_clearpks, POS_DEAD, 2, LOG_NORMAL, 0, 0, 0},
    { "clearvam",		do_clearvam,POS_STANDING,	 1,  LOG_NORMAL, 0,0,0	},
    { "clearvamp",	do_clearvamp,POS_STANDING,	 11,  LOG_ALWAYS, 0,0,0	},
    { "kinkboot",	do_kinkboot, POS_DEAD,	11, LOG_ALWAYS, 0, 0, 0},
    ...
    { "divorce",		do_divorce,	POS_DEAD,	 	 7,  LOG_ALWAYS, 0,0,0	},
    { "freeze",		do_freeze,	POS_DEAD,		 9,  LOG_ALWAYS, 0,0,0	},
    { "marry",		do_marry,	POS_DEAD,	 	 7,  LOG_ALWAYS, 0,0,0	},
    ...
    { "ntrust",		do_ntrust,	POS_DEAD,		 9,  LOG_ALWAYS, 0,0,0	},
    ...
    { "users",		do_users,	POS_DEAD,	 	 7,  LOG_NORMAL, 0,0,0	},
    { "omni",		do_omni,	POS_DEAD,		 7, LOG_NORMAL, 0,0,0 },
    { "wizlock",		do_wizlock,	POS_DEAD,		 11,  LOG_ALWAYS, 0,0,0	},
    { "watcher",		do_watcher,	POS_DEAD,	 	 12,  LOG_NEVER, 0,0,0	},
    { "force",		do_force,	POS_DEAD,	 	 9,  LOG_ALWAYS, 0,0,0	},
	/* jedi junk  Listy*/
	{ "channelforce", do_channelforce, POS_SITTING, 3, LOG_NORMAL, 0,0,0 },
        { "jstep",	do_jstep,	POS_STANDING,    3,  LOG_NORMAL, 0,0,0 },
	{ "forcetalk",	do_forcetalk,	POS_DEAD,		2,	LOG_NORMAL, 2048,0,0 },
	{ "<",			do_forcetalk,	POS_DEAD,		2,	LOG_NORMAL, 2048,0,0 },
	{ "apprentice", do_apprentice,	POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jedi",		do_jedi,		POS_STANDING,	3,	LOG_NORMAL, 2048,0,0 },
	{ "jediskill",	do_jediskill,	POS_DEAD,		3,	LOG_NORMAL, 2048,0,0 },
	{ "sense",		do_sense,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "initiate",	do_initiate,	POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "learn",		do_learn,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jedisleep",	do_jedisleep,	POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jedishield",	do_jedishield,	POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "fshield",	do_fshield,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "fspeed",		do_fspeed,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jtouch",		do_jtouch,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jsight",		do_jsight,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "make",		do_make,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jsense",		do_jsense,		POS_STANDING,	3,	LOG_NORMAL, 2048,0,0 },
	{ "jeyes",		do_jeyes,		POS_STANDING,	3,	LOG_NORMAL, 2048,0,0 },
	{ "jfocus",		do_jfocus,		POS_STANDING,	3,	LOG_NORMAL, 2048,0,0 },
	{ "jlevitate",	do_jlevitate,	POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jhealing",	do_jhealing,	POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jwalk",		do_jwalk,		POS_STANDING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jstate",		do_jstate,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jsummon",	do_jsummon,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jfight",		do_jfight,		POS_FIGHTING,	3,	LOG_NORMAL, 2048,0,0 },
	{ "jsault",		do_jsault,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jreverse",	do_jreverse,	POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jkneel",		do_jkneel,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jstab",		do_jstab,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jhigh",		do_jhigh,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jupper",		do_jupper,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jbutterfly",	do_jbutterfly,	POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jcross",		do_jcross,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jslash",		do_jslash,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	{ "jcircle",	do_jcircle,		POS_FIGHTING,	3,	LOG_NORMAL,	2048,0,0 },
	/* End of Jedi */
	{ "katana",	do_katana, POS_STANDING, 3, 0, 0, 0 },
    /*{ "vlist",		do_vlist,	POS_DEAD,		 11, LOG_NORMAL, 0,0,0 },*/
    ...
    { "pload",		do_pload,	POS_DEAD,		 0, LOG_ALWAYS, 0,0,0	},
    ...
    { "silence",		do_silence,	POS_DEAD, 10,  LOG_ALWAYS, 0,0,0 },
   ...
   /* { "deathblow",      do_deathblow,   POS_DEAD,         0,
LOG_NORMAL,
1}, */
    { "decapitate",	do_decapitate,POS_STANDING,	 3,  LOG_NORMAL,
0,0,0 },
    ...
    { "echo",		do_echo,	POS_DEAD,	 	 10,  LOG_ALWAYS, 0,0,0	},
    { "jail",           do_jail
,        POS_DEAD,                11,
LOG_ALWAYS, 0,0,0       },
    { "font",           do_font,        POS_DEAD,                 11, 
LOG_NORMAL, 0,0,0       },
    { "bonus",          do_bonus,       POS_DEAD,                 11,
LOG_ALWAYS, 0,0,0       },
    { "trivia",         do_trivia,       POS_DEAD,                11,
LOG_NORMAL, 0,0,0       },
    { "questinfo",      do_questinfo,    POS_DEAD,                11,
LOG_NORMAL, 0,0,0       },
    { "mudc",           do_mudc,         POS_DEAD,                12,
LOG_NORMAL, 0,0,0       }, 
    { "goto",		do_goto,	POS_DEAD,	 	 7,
LOG_NORMAL, 0,0,0	},
    ...
    { "magecolor",      do_magecolor,		POS_STANDING,	10, LOG_ALWAYS, 0,0,0 },
    ...
    { "die",		do_die,	POS_DEAD,	0, LOG_NORMAL },

    { "immtalk",		do_immtalk,	POS_DEAD,	 7,  LOG_NORMAL, 0,0,0	},
    { ":",			do_immtalk,	POS_DEAD,	 	 7,  LOG_NORMAL, 0,0,0	},

    { "magearmor",	do_magearmor,	POS_STANDING,   3, LOG_NORMAL, CLASS_MAGE,0,0 },
    { "monkcreate",	do_monkcreate,	POS_STANDING,	3, LOG_NORMAL, CLASS_MONK,0,0 },
    { "demonic",	do_demonic,	POS_STANDING,	3, LOG_NORMAL, CLASS_DEMON,0,0 },
    { "hellish",	do_hellish,	POS_STANDING,	3, LOG_NORMAL, CLASS_DEMON,0,0 },
    { "vampcreate",	do_vampcreate,	POS_STANDING,	3, LOG_NORMAL, CLASS_VAMPIRE,0,0 },
    { "firewalk",	do_firewalk,	POS_STANDING,	3, LOG_NORMAL, 0,0,0},
   
    /* Hunter Commands */
   // { "edge",           do_edge,        POS_STANDING,   2, LOG_NORMAL,
   // 0,0,0},
    { "veil",           do_veil,        POS_STANDING,   2, LOG_NORMAL,
    0,0,0},
    { "blend",          do_blend,       POS_STANDING,   2, LOG_NORMAL,
    0,0,0},
   /* { "huntalk",        do_hunttalk,    POS_DEAD,       1, LOG_NORMAL,
    0,0,0}, */
    { "radiate",        do_radiate,     POS_STANDING,   2, LOG_NORMAL,
    0,0,0},
   /* { "donatelife",         do_donatelife,      POS_STANDING,   2,
LOG_NORMAL, 
    0,0,0}, */
    { "bluster",        do_bluster,     POS_STANDING,   3, LOG_NORMAL,
    0,0,0},
  /*  { "smite",          do_smite,      POS_STANDING,   3, LOG_NORMAL,
    0,0,0}, */
    /* Unicorn Commands */
    { "sing",		do_sing,	POS_RESTING,	2, LOG_NORMAL},
    { "raise",		do_raise,	POS_RESTING,	2, LOG_NORMAL},

    /* Moogle commands */
    { "dance",		do_dance,	POS_RESTING,	2, LOG_NORMAL},

    /* new fight code commands */
    { "mock",		do_mock,	POS_DEAD,	7, LOG_NORMAL},
    { "newkill",	do_newkill,	POS_STANDING,	7, LOG_NORMAL},

    /* Golem Commands */
    {  "bazmapunch",	do_bazmapunch,	POS_FIGHTING,	2, LOG_NORMAL},
    {  "elbowbash",	do_elbowbash,	POS_FIGHTING,	2, LOG_NORMAL},
    {  "groinkick",	do_groinkick,	POS_FIGHTING,	2, LOG_NORMAL},
    {  "backslash",	do_backslash,	POS_FIGHTING,	2, LOG_NORMAL},
    {  "sumosplash",	do_sumosplash,	POS_FIGHTING,	2, LOG_NORMAL},
    {  "neckcrack",	do_neckcrack,	POS_FIGHTING,	2, LOG_NORMAL},
    {  "rocktalk",	do_rocktalk,	POS_DEAD,	0, LOG_NORMAL},
    {  "solidify",	do_solidify,	POS_RESTING,	0, LOG_NORMAL},
    {  "harden",	do_harden,	POS_RESTING,	0, LOG_NORMAL},
    {  "hitch",		do_hitch,	POS_STANDING,	0, LOG_NORMAL},
    {  "bind",		do_golbind,	POS_STANDING,	0, LOG_NORMAL},
    {  "modify",	do_modify,	POS_STANDING,   0, LOG_NORMAL},
    {  "texturize",	do_texturize,	POS_STANDING,	0, LOG_NORMAL},
    {  "bend",		do_study,	POS_STANDING,	0, LOG_NORMAL},

    /* Shapeshifting commands */
    { "shapecreate",	do_shapecreate, POS_STANDING,	3, LOG_NORMAL,0,0,0},
    { "randomize",	do_randomize,	POS_STANDING,	2, LOG_NORMAL, 0, 0, 0 },
//    { "beeping",	do_beeping,	POS_SLEEPING,	2, LOG_NORMAL, 0,
//0, 0 },
    { "double",		do_double,	POS_RESTING,	2, LOG_NORMAL, 0, 0, 0 },
    { "leave",		do_leave,	POS_STANDING,	2, LOG_NORMAL, 0, 0, 0 },
    { "warcry",		do_warcry,	POS_STANDING,	2, LOG_NORMAL, 0, 0, 0 },
    { "talents", 	do_talents,	POS_RESTING, 	2, LOG_NORMAL, 0, 0, 0 },
    { "toggle", 	do_toggle, 	POS_DEAD, 	2, LOG_NORMAL, 0, 0, 0 },
    { "shapetalk", 	do_shapetalk, 	POS_DEAD, 	2, LOG_NORMAL, 0, 0, 0 },
    { "shift", 		do_shift, 	POS_STANDING, 	2, LOG_NORMAL, 0, 0, 0 },
    { "shiftgate",	do_shiftgate,	POS_STANDING,	2, LOG_NORMAL, 0, 0, 0 },
    { "sinduct",	do_sinduct,	POS_STANDING,	2, LOG_NORMAL, 0, 0, 0 },    
    { "sexpel",		do_sexpel,	POS_STANDING,	2, LOG_NORMAL, 0, 0, 0 },    

    /* All Amazon commands in one big bunch, Adreno */
   { "amaztalk",	do_amaztalk,	POS_DEAD,  	 1, LOG_NORMAL, 0,0,0 },
    { "seduce",		do_seduce,	POS_FIGHTING,	 3, LOG_NORMAL, 0,0,0 },
    { "flare",		do_flare,	POS_STANDING,    3, LOG_NORMAL, 0,0,0 },
    { "steelfist",	do_steelfist,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "succour",	do_succour,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "drill",		do_drill,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 }, 
    { "sisters",	do_sisters,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "genesis",	do_genesis,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "cognizance",	do_cognizance,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "camouflage",	do_camouflage,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "barkskin",	do_barkskin,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "net",		do_net,		POS_FIGHTING,	 3, LOG_NORMAL, 0,0,0 },
    { "combo",		do_combo,	POS_STANDING,    3, LOG_NORMAL, 0,0,0 },
    { "spinkick",	do_spinkick,	POS_FIGHTING,	 3, LOG_NORMAL,	0,0,0 },
    { "sweepkick",	do_sweepkick,	POS_FIGHTING,	 3, LOG_NORMAL, 0,0,0 },
    { "axekick",	do_axekick,	POS_FIGHTING,	 3, LOG_NORMAL, 0,0,0 },
    { "forearm",	do_forearm,	POS_FIGHTING,	 3, LOG_NORMAL, 0,0,0 },
    { "backfist",	do_backfist,	POS_FIGHTING,	 3, LOG_NORMAL,	0,0,0 },
	{ "praise",		do_praise,		POS_STANDING,	3, LOG_NORMAL, 0,0,0 },
{ "slaveinsight", do_slaveinsight, POS_STANDING, 3, LOG_NORMAL, 0,0,0},
{ "amazonworship", do_amazonworship, POS_STANDING, 3, LOG_NORMAL, 0,0,0},
    { "slavecommand",	do_slavecommand,POS_STANDING,	3, LOG_NORMAL, 0,0,0},
/* highlander stuff Adreno */
 /*   { "observe",	do_observe,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
    { "classify",	do_classify,	POS_STANDING,	 3, LOG_NORMAL, 0,0,0 },
...
const	struct	social_type	social_table [] =
{
    ...
    {
	"honey",
	"You bring out a jar of honey.",
	"$n brings out a jar of honey.",
	"You cover $N in honey and burry $M in the sand.",
	"$n covers $N in honey and burries $M in the sand.",
	"$n covers you in honey and burries you in the sand.",
	"You cover yourself in honey and try to lick it off.",
	"$n covers $mself in honey, and tries to lick it off."
    },
	...
	"You huggle whom?",
	NULL,
	"You smother $M in warm huggles.",
	"$n huggles $N.",
	"$n smothers you in loads of warm huggles.",
	"You huggle yourself.",
	"$n huggles $mself, maybe you should huggle them instead"
    },
...

    {
        ...
        "You mumble at $M",
        "$n mumbles at $N",
        "$n mumbles at you, what did you do wrong?",
        "You mumble to yourself, you must be talking to the voices in your head",
        "$n mumbles at $mself. $e must be hearing voices again"
    },
	...
	"You mutter at $M",
	"$n mutters at $N",
        "$n mutters in your direction, what happened?",
        "You mutter to yourself, that really is the first sign of madness you know.",
        "$n mutters at $mself. $e is probably going mad"
    },	

    {
        "smack",
        "You lay the smack down on someone.",
        "$n pretends to lay the smack down.",
        "You smack $M, must've deserved it.",
        "$n smacks $N, probably deserved it to happen.",
        "$n smacks you down, you feel like a jabronie.",
        "You smack yourself heavily about the head.",
        "$n smacks $mself, saves you doing it I suppose."
    },

    {
        "muffin",
        "You offer everyone in the room a hot tasty blueberry muffin.",
        "$n offers hot tasty blueberry muffins to the room, mmm they smell good.",
        "You give $M a hot tasty chocolate chip muffin.",
        "$n gives $N a flavoursome banana nut muffin, you want one too.",
        "$n offers you a warm fresh raspberry cream muffin.",
        "You give yourself a tasty carrot cake muffin.",
        "$n gives $mself a wonderful smelling dutch cocoa muffin, you feel hungry."
    },

    {
        "hock",
        "You spit on the ground in disgust.",
        "$n spits on the ground in disgust, I hope they clean it up.",
        "You spit at $M, must have done something really bad.",
        "$n spits at $N, I think there might be a fight here.",
        "$n spits at you, I think you should kill him now.",
        "You spit at yourself?.",
        "$n spits at $mself, that's just plain weird."
    },

    {
        "shudder",
        "You shudder violently.",
        "$n shudders a little, wonder what they just heard.",
        "You shudder at $M, for what they just did.",
        "$n shudders at $N, can't have been overly pleasant.",
        "$n shudders at what you did.",
        "You shudder coldly to yourself.",
        "$n shudders at $mself, wonder what they just heard."
    },

    { 
        "ugh",
        "You ugh loudly, heavy night last night?.",
        "$n ughs loudly. You think they had a heavy night.",
        "You ugh at $M, did they try and wake you up?.",
        "$n ughs at $N, better stay out of the way.",
        "$n ughs at you, you tried to wake $m up didn't you?",
        "You ugh loudly at yourself, back to bed then.",
        "$n ughs at $mself, you think this is a good time to run."
    },
    ...
    {
	"mhfnog",
	"You take a moment to let out some hard fucking nogs!.",
	"$n takes a moment to let out some hard fucking nogs!.",
	"You take a moment to let out some hard fucking nogs at $M.",
	"$n takes a moment to let out some hard fucking nogs at $N.",
	"$n takes a moment to let out some hard fucking nogs at you.  You better agree with $m!",
	"You take a moment to let out some hard fucking nogs at yourself. You're whacked dude!",
	"$n takes a moment to let out some hard fucking nogs at $mself. $e is really whacked!"
    },
...
void interpret( CHAR_DATA *ch, char *argument )
{
    ...
    */ 
	...
	&&
(is_pure(cmd_table[cmd].name,ch->immpower) || cmd_table[cmd].level <= trust ))
	{
    ...
    {
	sprintf( log_buf, "Log %s: %s", !IS_NPC (ch) ?
ch->pcdata->switchname : ch->short_descr, logline );
	log_string( log_buf );
    }
	 ...
	 */
	if ( !check_social( ch, command, argument,TYPE_SOCIAL ) )
	{
...
*/
	else if (!str_prefix("lick",command) && IS_GOLEM(ch) && ch->hit > 0)
	{CHAR_DATA *vict; bool heal = TRUE;int dam;
	    if ((vict = get_char_room(ch, argument)) == NULL
		||!*argument)
		vict = ch->fighting ? ch->fighting : ch;
	    if (vict->fighting == ch || ch->fighting == vict)
		heal = FALSE;
	    dam = NR(1000,1700);
	    if (vict == ch) dam /= 3;
	    if (!heal) {dam = 0-dam;dam /= 2.5;}
	    if (heal)
	    {
		act("#6You feel better!#n",vict,0,0,TO_CHAR);
	    if (ch != vict)
		stc("#6You cover them in love.#n\n\r", ch);
		act("#6as $n licks $N, $M pain fades.",ch,0,vict,TO_NOTVICT);
	    }
	    if (!heal)
	    {
		act("#1$n's lick covers $N in gruesome slime.",ch,0,vict,TO_NOTVICT);
		stc("#1Your lick covers them in gruesome slime.\n\r",ch);
		act("#1$n's lick covers you in guesome slime.",ch,0,vict,TO_VICT);
	    }
	    WAIT_STATE(ch, ch->generation < 7 ? 7 : ch->generation);
	    smooregen(vict, dam, 100, 100);
	}
	return;
    }
    else /* a normal valid command.. check if it is disabled */
      if (check_disabled (&cmd_table[cmd]) &&
!is_pure(cmd_table[cmd].name, ch->immpower))
      {
              send_to_char ("This command has been temporarily disabled.\n\r", ch );
             return;
      }
    else if (is_pure(cmd_table[cmd].name, ch->in_room->disabled) && !IS_IMMORTAL(ch))
    {
	stc("The power..its too strong..its holding you back!\n\r", ch);
	return;
    }
    if (bugfind)
{
...
extern int minute;
    ...
    sprintf(strsave, "../bugfind/%d.txt",minute);
	if ( (bug = fopen(strsave,"w")) != NULL )
   {
	sprintf(buf,"%s %s %s", ch->name, cmd_table[cmd].name, argument);
  ...
  fprintf(bug, buf);
     ...
     */if (!is_pure(cmd_table[cmd].name, ch->immpower)) {
    if ( ch->position < cmd_table[cmd].position )
    {
	switch( ch->position )
	{
     ...
     */
free_string(last_user);
last_user = str_dup(ch->name);
free_string(last_command);
sprintf(log_buf, "%s %s", cmd_table[cmd].name, argument);
last_command = str_dup(log_buf);
command_finished = FALSE;

    (*cmd_table[cmd].do_fun) ( ch, argument );
if (IS_GOLEM(ch) && IS_INFORM(ch,GOLEM_RUBY))
{
 if (cmd_table[cmd].position > POS_SLEEPING) {
  if (!(ch->moves & IN_RUBY)) {
    act("#6$n melts and reforms into $t.#n", ch, ch->name, 0, TO_ROOM);
    stc("#1Your quick actions make you hot, forcing you to melt and reform.#n\n\r", ch);
    free_string(ch->morph);
    ch->morph = str_dup("");
    ch->morphy = FALSE;
    NO_STONE(ch);
  } else REMOVE_BIT(ch->moves, IN_RUBY);
 }
}
...
bool check_social( CHAR_DATA *ch, char *command, char *argument,int type)
{
    char arg[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    int cmd;
...
else
{
    if ( arg[0] == '\0' )
    {
	sprintf(log_buf,"#2[#1GSocial#2] #6%s",
social_table[cmd].others_no_arg);
	gsact( log_buf, ch, NULL, victim, TO_ROOM    );
	sprintf(log_buf,"#2[#1GSocial#2] #6%s",
social_table[cmd].char_no_arg);
	gsact( log_buf, ch, NULL, victim, TO_CHAR    );
    }
...
social_table[cmd].others_found);
	gsact( log_buf, ch, NULL, victim, TO_NOTVICT    );
	sprintf(log_buf,"#2[#1GSocial#2] #6%s",
social_table[cmd].char_found);
	gsact( log_buf, ch, NULL, victim, TO_CHAR    );
	sprintf(log_buf,"#2[#1GSocial#2] #6%s",
social_table[cmd].vict_found);
	gsact( log_buf, ch, NULL, victim, TO_VICT    );
    }
...

{
	...
	fp = fopen ("ban.txt", "r");
        ...
        if (!fp) 
        {
          bug("could not open ban.txt for load",0);
	save_bans();
          return;
        }  

for (name = fread_word(fp); str_cmp(name, END_MARKER); name = fread_word(fp))
{
...
void do_findbug(CHAR_DATA *ch, char *argument)
{
if (IS_IMMORTAL(ch) || str_cmp(ch->name, "Smoo")) {
    if (bugfind) bugfind = FALSE;
    else    if (!bugfind) bugfind = TRUE;
if (bugfind) send_to_char("On.\n\r", ch);else send_to_char("Off.\n\r", ch);
}
