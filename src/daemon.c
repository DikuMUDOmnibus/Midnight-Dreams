...
void do_transform( CHAR_DATA *ch, char *argument )
{
    ...
    free_string(ch->morph);
sprintf(log_buf,"#1[#2%s#1]#6%s", ch->pcdata->switchname, obj->short_descr);
...
void do_stigmata( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		...
		{
			touchdamage = number_range(1500,(ch->move / 3));
			ch->move -= touchdamage + 4000 ;
                  ch->fight_timer += 10;
	     	}
	
		else
		{
			touchdamage =number_range(1500,(ch->move / 2.97));
        	    ch->move -= ch->move/3;
                ch->fight_timer += 10;
 		}
touchdamage *= 1.4;
if (IS_NPC(victim)) touchdamage *= 2;	
		if (touchdamage > 0)
		{
...
void do_hellish( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->pcdata->stats[DEMON_TOTAL] < 7500 || ch->pcdata->stats[DEMON_CURRENT] < 7500)
    {
	send_to_char("It costs 7500 points of power to create a piece of hellish armour.\n\r",ch);
	return;
    }
    if ( vnum == 0 || (pObjIndex = get_obj_index( vnum )) == NULL)
    {
    ...
    ch->pcdata->stats[DEMON_TOTAL]   -= 7500;
    ch->pcdata->stats[DEMON_CURRENT] -= 7500;
    obj = create_object(pObjIndex, 50);
    obj_to_char(obj, ch);
    act("$p appears in your hands in a blast of flames.",ch,obj,NULL,TO_CHAR);
    act("$p appears in $n's hands in a blast of flames.",ch,obj,NULL,TO_ROOM);
...
{CHAR_DATA *victim, *vch, *vch_next;
 ...
 if (ch->move < 2500)
 {
    stc("You need 2500 movement points to use that
\r(common, I made it good!)\n\r", ch);
    return;
 }

 ch->move -= 2000; // leave them some mv to flee with
 ...
 act("#3$n pops out of existance.#n", ch,0,0,TO_ROOM);
 stc("#3You pop out of existance.\n\r", ch);
