...

{
	...
	act("$n glows slightly as they mutter ancient texts if power.",ch,NULL,NULL,TO_ROOM);
...
void do_advocate (CHAR_DATA *ch, char *argument)
{
	...
	if (ch->level == LEVEL_APPRENTICE)
	{
		if (ch->exp < 500000)
		{
			send_to_char("You do not have the 500000 experience to become a mage.\n\r", ch);
			return;
		}
		if (ch->max_mana < 10000)
		{
			send_to_char("You mana level has not reached the 10000 required.\n\r", ch);
			return;
		}
		if (ch->max_hit < 5000)
		{
			send_to_char("You hit point level has not reached the 5000 required.\n\r", ch);
			return;
		}
		if (mastered < 2)
		{
	...
	else if (ch->level == LEVEL_MAGE)
	{
		if (ch->exp < 2000000)
		{
			send_to_char("You do not have the 2000000 experience to become a mage.\n\r", ch);
			return;
		}
		if (ch->max_mana < 17500)
		{
			send_to_char("You mana level has not reached the 17500 required.\n\r", ch);
			return;
		}
		if (ch->max_hit < 12500)
		{
 ...
 void do_soulbond (CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->exp < 5000000)
    { 
      send_to_char("You require 5 million experience to transcend.\n\r", ch);
      return;
    }
    ch->exp -= 5000000;
...
void do_necromancy(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->exp < 10000000)
    {
      send_to_char("You do not have the required 10 million exp to study.\n\r", ch);
      return;
    }
    ch->exp -= 10000000;
...
void do_illusion(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->exp < 10000000)
    {
      send_to_char("You do not have the required 10 million exp to study this profession.\n\r", ch);
      return;
    }
    ch->exp -= 10000000;
...
void do_invocation(CHAR_DATA *ch, char *argument)
{
   ...
   if (ch->exp < 10000000)
   {
     send_to_char("You do not have the required 10 million experience to study this profession.\n\r", ch);
     return;
   }
   ch->exp -= 10000000;
...
*/
    ...
    spellcount = ((page->value[1] * 5000) + (page->value[2] * 10) + 
...
if (victim_target && ch != victim && IS_SET(ch->act, PLR_SACREDINVIS) )
	...
	enhance_stat(sn,level,ch,victim,APPLY_HITROLL,bonuses * 0.6,affect_bit);
	...
	enhance_stat(sn,level,ch,victim,APPLY_DAMROLL,bonuses * 0.6,affect_bit);
...
void do_undeadcall(CHAR_DATA *ch, char *argument)
{
    ...
    victim->level = value * 15;
    victim->armor = 0 - (20*value);
    victim->hitroll = value*10;
    victim->damroll = value*10;
...
void adv_spell_stack( CHAR_DATA *ch, OBJ_DATA *book, OBJ_DATA *page, char *argument)
{
 ...
 if (++stack_count > 3)
 {
     stc("Failed. You may only have 3 none-true magic spells stacked.\n\r", ch);
     return;
 }
...
ch->mana -= manacost;
 ...
 if (IS_SET(page->spectype, ADV_NEXT_PAGE)) {
	if ( ( page_next = get_page( book, page->value[0]+1 ) ) == NULL )
	{send_to_char( "The spell failed.\n\r", ch );return;}
    	if (IS_SET(page_next->quest, QUEST_MASTER_RUNE))
    	{
	    if      (IS_SET(page_next->spectype, ADV_DAMAGE))
