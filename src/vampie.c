...
{
	send_to_char("You don't have enough blood for that. 200 needed.\n\r", ch);
	return;
}
...
if (!str_cmp(arg,"n")) sprintf(wall,"kinkn");
if (!str_cmp(arg,"w")) sprintf(wall,"kinkw");
if (!str_cmp(arg,"s")) sprintf(wall,"kinks");
if (!str_cmp(arg,"e")) sprintf(wall,"kinke");
if (!str_cmp(arg,"d")) sprintf(wall,"kinkd");
if (!str_cmp(arg,"u")) sprintf(wall,"kinku");
...
if (!str_cmp(arg,"n"))
{
obj=create_object(get_obj_index(23),0);
...
*/
	...
	if (ch->level < 3)
	{
		send_to_char("You cannot frenzy whilst mortal.\n\r", ch);
		return;
	}
...
void do_drain( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->power[DISC_VAMP_DAIM] < 4)
     {
	send_to_char("You need at least level 4 Daimoinon to use this power.\n\r",ch);
	return;
     }
   ...
   if ( arg[0] == '\0' && (ch->fighting) == NULL)
    ...
    if (IS_NPC(victim))
	{
	  dam = ch->power[DISC_VAMP_DAIM] * 80;
        victim->hit = (victim->hit - dam);
        if (dam > 2300) dam = 2300;
        ch->hit = (ch->hit + dam);
    ...
    if (!IS_NPC(victim))
	{
	  dam = ch->power[DISC_VAMP_DAIM] * 50;
        victim->hit = (victim->hit - dam);
        if (dam > 2300) dam = 2300;
        ch->hit = (ch->hit + dam);
        if (ch->hit > ch->max_hit + 1000)
...
void do_cauldron( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->power[DISC_VAMP_THAU] < 5)
     {
	send_to_char("You need at least level 5 Thaumaturgy to use this power.\n\r",ch);
	return;
     }
