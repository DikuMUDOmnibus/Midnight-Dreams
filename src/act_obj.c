...
void do_drink( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC(ch) && !IS_CLASS(ch, CLASS_VAMPIRE) &&
		ch->pcdata->condition[COND_FULL]   > 2000 )
	    send_to_char( "You are full.\n\r", ch );
	if ( !IS_NPC(ch) && !IS_CLASS(ch, CLASS_VAMPIRE) &&
		ch->pcdata->condition[COND_THIRST] > 1999 )
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
    ...
    if (IS_SET(obj->quest, ARMOR_ENCHANTED) && !IS_CLASS(ch, CLASS_MAGE))
	{
	act( "$p jumps out of $n's hand.", ch, obj, NULL, TO_ROOM );
	act( "$p jumps out of your hand.", ch, obj, NULL, TO_CHAR );
	obj_from_char(obj);
	obj_to_room(obj,ch->in_room);
	return;
	}
if (IS_CLASS(ch,CLASS_MONK) && obj->item_type == ITEM_WEAPON)
{stc("Monks can't wear weapons!\n\r", ch);return;}
...
/*
if (get_obj_index(OBJ_VNUM_TOOTH))      
{ 
        stc("You cant wear a tooth!!!.\n\r",ch);          
                  return;                                              
}
*/
...
void quest_object( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    {
	 102,
	 9201, 9225,  605, 1329, 2276, 5112, 6513, 6517, 2448, 5001,
...
void do_complete( CHAR_DATA *ch, char *argument )
{
	...
	if (qobj->value[0] != -1)
	{
	    ...
	    else sprintf(buf, "     Bugged Card %d.\r\n",qobj->value[0]);
	    buf[5] = UPPER(buf[5]);
	    send_to_char(buf,ch);
	}
	if (qobj->value[1] != -1)
    ...
    {
	send_to_char("That cloned object is not usable.\n\r",ch);
	return;
    }
    if (qobj->value[0] != -1)
    {
