...
void do_setdisciplines(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg1[0] == '\0' || arg2[0] == '\0' || (!is_number(arg3)&&*arg3 !=  '-'))
    {
	send_to_char("Syntax: Setdisciplines <char> <discipline/all> <num>\n\r",ch);
	return;
    }
...
void logchan( char *argument )
{
	...
	if ( (d->connected == CON_PLAYING) &&(( IS_JUDGE(ch) && 
	    !IS_SET(ch->deaf,CHANNEL_LOG)) || is_pure("log",ch->immpower) ) )
	{
	    send_to_char( "[", ch );
	    send_to_char( argument, ch );
	    send_to_char( "]\n\r",  ch );
...
void do_echo( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected == CON_PLAYING )
	{
	    send_to_char("#5Echo ->#n",d->character);
	    send_to_char( argument, d->character );
	    send_to_char( "\n\r",   d->character );
	}
    }
...
void do_transfer( CHAR_DATA *ch, char *argument )
{
    ...
    {int iA;char *arg = arg1;
	arg+=4;
	sprintf(buf, "1.%s", arg);
	for (iA = 1; get_char_world(ch, buf); iA++)
	{
	    do_transfer(ch, buf);
	    sprintf(buf, "%d.%s", iA+1, arg);
	}
	return;
    }
...
if (IS_NPC(victim)) {stc("Not on MOBS!\n\r", ch);return;}
    sprintf( buf, "Player Statistics For %s.\n\r", victim->name );
    send_to_char( buf, ch );
	...
	send_to_char( buf, ch);
	sprintf(buf, "Quests done : %4d Total Questing Value : %5d\n\r", victim->pcdata->score[SCORE_NUM_QUEST], victim->pcdata->score[SCORE_QUEST]);
	send_to_char( buf, ch);
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	get_age( victim ), (int) victim->played + (int) (current_time -
	victim->logon) , victim->timer, victim->act );
    send_to_char( buf, ch );
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "\n\r#4You Wake up screaming as your mom Drenches you in water and screams #3WAKE UP#n.\n\r");
...
void do_snoop( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( victim ) >= get_trust( ch ) && !is_pure("snoop",ch->immpower))
    {
	send_to_char( "You failed.\n\r", ch );
	return;
    }
...
if (IS_NPC(ch)) return;
 ...
 if (!ch->pload || !str_cmp(ch->pload, ""))
 {
if (!is_pure("pload",ch->immpower) && ch->level < 12) {
    stc("Huh?\n\r",ch);return;}}
...
void do_trust( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "Trust being one of: None, Mortal, Builder,
Questmaker, Enforcer, Judge, or Highjudge.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Please enter: None, Mortal, Builder, Questmaker,
Enforcer, Judge, or Highjudge.\n\r", ch );
	return;
    }
...
void do_restore( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (get_trust(ch) >=  MAX_LEVEL - 2 ||
is_pure("restore",ch->immpower)) && !str_cmp(arg,"all"))
    {
    /* cure all */
        for (d = descriptor_list; d != NULL; d = d->next)
        {
...
void do_force( CHAR_DATA *ch, char *argument )
{
	...
	if (get_trust(ch) < MAX_LEVEL - 3 && !is_pure("force",ch->immpower))
	{
...
void do_otransfer( CHAR_DATA *ch, char *argument )
{
    ...
    {
if (!is_pure("otransfer",ch->immpower)) {
...
void do_bind( CHAR_DATA *ch, char *argument )
{
    ...
    OBJ_DATA *obj;
    CHAR_DATA *victim;
if (IS_GOLEM(ch) && ch->level < 7)
{do_golbind(ch,argument);return;}
    argument = one_argument( argument, arg1 );
    argument = one_argument( argument, arg2 );
