...

{
	...
	if ( IS_CLASS(victim, CLASS_MAGE) && (value < 5 || value > 275 ) )

	{

	    send_to_char( "Spell range is 5 to 275.\n\r", ch );

	    return;

	}

	else if ( value < 5 || value > 295 )
...

	{
	    ...
	    send_to_char( "Spell range is 5 to 295.\n\r", ch );
...
void do_paradox( CHAR_DATA *ch, char *argument)  
{
	...
	{
		send_to_char( "You cannot paradox them.\n\r", ch);
		return;
	}
	paradox(victim);
	send_to_char("Done!\n\r", ch );
