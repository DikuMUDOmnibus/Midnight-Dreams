...
void load_total( void )
{
    FILE *fp;
    int   number = 0;

    if ( !( fp = fopen( TOTAL_FILE, "r" ) ) )
        return;
    ...
    for ( ; ; )
    {
		...
		number = 0;
		if ( !str_cmp( word, "TOTAL_COMPLETE" ) )
        {
            number = fread_number( fp );
            if ( number > 0 )
