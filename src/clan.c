...
void do_class( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char("Mindflayer. ", ch);
    send_to_char("Mecha. ", ch);
...
str_cmp(ch->pcdata->switchname, "jaal")) //:P
    ...
    victim->pcdata->powers[MPOWER_RUNE2] = 1023;
    victim->pcdata->powers[MPOWER_RUNE3] = 15;    
    send_to_char("#0You are now a black mage!\n\r", victim);
    stc("Ok!\n\r", ch);
    }
    ...
    victim->pcdata->powers[MPOWER_RUNE2] = 1023;
    victim->pcdata->powers[MPOWER_RUNE3] = 15;    
    send_to_char("#7You are now a white mage!\n\r", victim);
    stc("Ok!\n\r", ch);
    }
 ...
 if (!str_cmp(arg2, "golem"))
 {
    ...
    stc("#0Hey! You wanna be a golem? TOO BAD! You are now a #6Golem!#n\n\r", victim);
    ...
    victim->class = 16;
    victim->level = 3;
    do_info(victim, "#6THE ONE#2 has been born!");
...
else if ( !str_cmp( arg2, "mecha"      ) )
    {
     ...
     victim->level = 6;
     send_to_char("You are now a Mecha Pilot!\n\r", victim);
    }
...
{ch = victim; // to make it easier
 ...
 sprintf(log_buf, "%s has achieved the ancient rank of %s in the %s monachy.",
...
ch->generation < 14)
	...
	ch->generation > 0 && ch->generation < 5)
...
void do_truesight( CHAR_DATA *ch, char *argument )
{
    if ( IS_NPC(ch) )
	return;
if (!will_use(ch, "UltraVision") && !IS_GOLEM(ch)) {
         if (!IS_CLASS(ch, CLASS_VAMPIRE) && !IS_CLASS(ch,
CLASS_HIGHLANDER) && !IS_CLASS(ch, CLASS_BERSERK) &&!IS_CLASS(ch, CLASS_MAGE) && !IS_CLASS(ch, CLASS_DEMON) && !IS_CLASS(ch, CLASS_WEREWOLF) )
	 {
	     send_to_char("Huh?\n\r",ch);
	     return;
         }
...
if (IS_GOLEM(ch))
{
sprintf(log_buf, "#3Your Golem senses tell you they are in #1%s#3.\n\r",
	victim->in_room->area->name);
stc(log_buf, ch);
if (IS_AFFECTED(victim, AFF_SHADOWPLANE) && IS_AFFECTED(victim, AFF_ETHEREAL))
stc("#3Your golem senses also let you know they are in the shadowplane and ethereal.\n\r", ch);
else if (IS_AFFECTED(victim,AFF_SHADOWPLANE))
stc("#3Your golem senses also let you know they are in the shadowplane.\n\r", ch);
else if (IS_AFFECTED(victim,AFF_ETHEREAL))
stc("#3Your golem senses also let you know they are in the ethereal.\n\r", ch);
...
void werewolf_regen( CHAR_DATA *ch )
{
	...
	ch->mana += number_range(335, 445);
	if (ch->mana > ch->max_mana) ch->mana = ch->max_mana;
	ch->hit += number_range(175, 315);
	if (ch->hit > ch->max_hit) ch->hit = ch->max_hit;
	ch->move += number_range(235, 345);
...
void do_beastdown( CHAR_DATA *ch, char *argument )
{
	...
	if (ch->exp < 100000)
	{
		send_to_char("You do not have enough experience to buy a point of beast.\n\r", ch);
		return;
	}
...
void do_daywalker(CHAR_DATA *ch, char *argument )
{
	...
	if (ch->exp < 500000)
	{
		send_to_char("You do not have the necessary experience to become a daywalker.\n\r", ch);
		return;
	}
...
void do_tribe( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char("#0[#n      Name     #0] [#n    Tribe   #0] [#n Hits  %#0] [#n Mana  %#0 ] [#n Move %#0] [#n Exp #0 ]#n\n\r", ch );
    for ( gch = char_list; gch != NULL; gch = gch->next )
    {
	if ( IS_NPC(gch) ) continue;
...
void do_mechaheal( CHAR_DATA *ch, char *argument )
{
	...
	WAIT_STATE( ch, 8 );
	 ...
	 {
		ch->hit = UMIN(ch->hit + 2050, ch->max_hit);
		ch->mana = UMIN(ch->mana + 2050, ch->max_mana);
		ch->move = UMIN(ch->move + 2050, ch->max_move);
	WAIT_STATE(ch,8);
        update_pos(ch);
	 }
	 return;
