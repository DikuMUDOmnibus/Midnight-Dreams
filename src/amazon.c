...
IS_SET(victim->pcdata->stats[UNI_AFF], VAM_MORTAL))
   ...
   if (IS_CLASS(victim, CLASS_NINJA))
    ...
    if (ch->exp < 1000000)
    {
	send_to_char("You cannot afford the 1000000 exp required!\n\r", ch);
    ...
    {
	send_to_char("They cannot afford the 1000000 exp required!\n\r",ch);
	return;
    }
...
void do_genesis( CHAR_DATA *ch, char *argument )
{
   ...
   if (arg[0] == '\0')
	...
	send_to_char("Mask, Bracer, Lorica, Gloves, Cape, Boots or Spear.\n\r", ch );
	return;
    }
    ...
    else if (!str_cmp(arg,"spear"    )) vnum = 27949;
    else
    {
