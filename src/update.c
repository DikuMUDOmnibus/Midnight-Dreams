...
/* ONLY SMOO OR BERELAIN WORK ON THIS I PATCHED BLOODLUST INTO FUCKING 
MOBILE UPDATE BECAUSE THIS FUCKING PIECE OF SHIT DONT WORK OK
void bloodlust_update (void)
{
  CHAR_DATA *ch = NULL;
  ...
  for(d = descriptor_list;d;d = d->next) {
   ...
   if (ch->bloodlust > 20)
    ...
    {
      stc ("You Scream Out In Rage and Whip your blade around Wildly.\n\r",
            ch);
        act ("$n Screams and Swings his blade Hitting You !!!", ch, NULL,
              NULL, TO_ROOM);
      do_berserk (ch, "auto");
    }
  if (IS_CLASS (ch, CLASS_BERSERK)
       && (ch->position == POS_FIGHTING && ch->bloodlust < 100))
    
    {
      stc ("Your Eyes seem to turn #1Red #1with #3BLOODLUST#n.\n\r", ch);
      ch->bloodlust += 2;
    }
  if (IS_CLASS (ch, CLASS_BERSERK) && (ch->position != POS_FIGHTING))
    {
      stc ("The #3Lust #nin your body seems to slow down...\n\r", ch);
      ch->bloodlust -= 6;
    }
  }
}
...
if (IS_NPC(ch) && ch->spec_fun == spec_lookup("dancing_gummy_bear") &&
spec_lookup("dancing_gummy_bear") != NULL && count >= 60)
{act("$n melts from the sun.", ch, 0, 0, TO_ROOM);
extract_char(ch, TRUE); continue;}

	if (ch->level < 3)
	smooregen(ch, 200, 200, 200);
	if (IS_CLASS(ch,CLASS_HUNTER) && ch->hit < 1)
		smooregen(ch,NR(1,4),NR(500,1200),NR(500,1200));
	else if (IS_CLASS(ch,CLASS_HUNTER) && ch->fighting)
		smooregen(ch,NR(300,450),NR(200,350),NR(200,350));
	else if (IS_CLASS(ch,CLASS_HUNTER) && ch->position != POS_STUNNED)
		smooregen2(ch,NR(500,950),NR(450,790),NR(450,790),450); 
...
{
stc("You Scream Out In Rage and Whip your blade around Wildly.\n\r",ch);
act("$n Screams and Swings his blade Hitting You !!!",ch,NULL,NULL,TO_ROOM); 
do_berserk(ch,"auto");
}*/
if (IS_CLASS(ch,CLASS_BERSERK) && (ch->position == POS_FIGHTING && ch->bloodlust < 100))
{
stc("Your Eyes seem to turn #1Red #1with #3BLOODLUST#n.\n\r",ch);
ch->bloodlust += 2;
}
if (IS_CLASS(ch,CLASS_BERSERK) && (ch->position != POS_FIGHTING) && ch->bloodlust)
{
stc("The #3Lust #nin your body seems to slow down...\n\r",ch);
ch->bloodlust -= 2;
}
			...
			if (ch->class == 0)
			{
				...
				smooregen(ch, number_range(50, 100), 70,70);

/* what is this here for? werewolf? well this is for classless people
	else if (ch->position == POS_SLEEPING &&
ch->power[DISC_WERE_BOAR]> 3)
				smooregen(ch, 999, 500, 500);
*/
				else smooregen(ch, 150,150,150);
				}
				else
				    mortal_regen(ch);
			}
...
/*				ch->hit += number_range(95, 135);
				ch->mana += number_range(75, 125);
				ch->move += number_range(75, 125);
*/
			...
			if (ch->move > ch->max_move) ch->move = ch->max_move;
				...
				/* Demon Hell Regen */
			    if (ch->in_room && (ch->in_room->vnum == 3 || ch->in_room->vnum == 6))
			    {
...
else if (( IS_CLASS(ch, CLASS_MECHA)&&IS_HERO(ch)) || (IS_CLASS(ch, CLASS_BERSERK)&&IS_HERO(ch) ))
			{
			if (ch->position != POS_FIGHTING && ch->hit > 0)
			smooregen(ch, 860, 450, 520);
				if ( ch->hit < 0)
				{
...
if ( ch->hit < ch->max_hit || ch->mana < ch->max_mana || ch->move < ch->max_move )
			...
			else if (IS_CLASS(ch, CLASS_MONK) &&IS_HERO(ch))
			{
				...
				{
					//wth ? 4 sec 750 HP?
//				smooregen(ch, 750, 500, 750);
				smooregen(ch, 300, 150, 300);
				}
...
ch, NULL, NULL, TO_ROOM);}
	if (will_use(ch, "Fast Healing") && ch->hit > 0) smooregen(ch, 60,
85, 50);
...
for (i = 0; i < MAX_KEY_HASH; i++)
{
	...
	if (str_cmp(rm->disabled, "") && --rm->disabletick < 0)
	{
		...
		stc("#6The power surrounding the room fades..\n\r",ch);
...
void total_update(void)
{
        ...
        if ( !( fp = fopen ( TOTAL_FILE, "w" ) ) )
        {
                bug( "MAX_ON:  fopen of Maxonlien failed", 0 );
                return;
        }
        fprintf (fp, "TOTAL_LOGIN %d\n",logins);
        fprintf (fp, "TOTAL_LEVEL %d\n",levels);
        fprintf (fp, "TOTAL_NEWBIES %d\n",newbies);
        fprintf (fp, "TOTAL_DELETE %d\n",deletions);
        fprintf (fp, "TOTAL_MOBS %d\n",mobdeaths);
		fprintf (fp, "TOTAL_AUC %d\n",auctions);
		fprintf (fp, "TOTAL_SOLD %d\n", sold);
		fprintf (fp, "TOTAL_PDIED %d\n", pdied);
		fprintf (fp, "TOTAL_PK %d\n", pk);
		fprintf (fp, "TOTAL_GOS %d\n", gossip);
		fprintf (fp, "TOTAL_ARENA %d\n", arena);
		fprintf (fp, "TOTAL_NOTES %d\n", tnotes);
		fprintf (fp, "TOTAL_QUESTS %d\n", quests);
		fprintf (fp, "TOTAL_COMPLETE %d\n", questcomplete);
		fprintf (fp, "TOTAL_OOC %d\n", ooc);
		fprintf (fp, "TOTAL_QP %d\n", qps);
		fprintf (fp, "TOTAL_HERO %d\n", thero);
		fprintf (fp, "TOTAL_REMORT %d", tremort);
        fclose(fp);
