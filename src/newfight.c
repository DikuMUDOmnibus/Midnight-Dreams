...
int getdamage(CD *ch, CD *vict, int damtype, int element)
{
 ...
 else
 {
	...
	if (ch->pcdata->powers[MPOWER_PSCIONICS] >= 1)
	    dam *= 1 + (ch->pcdata->powers[MPOWER_PSCIONICS] * .25);
    }
...
get_eq_char(vict2,WEAR_WIELD)->value[3] + TYPE_HIT : TYPE_HIT, 0);
    sprintf(log_buf, "#1%s's mock hit (new fight code) would have done #3%d#1 damage to %s.#n\n\r", PERS(vict2,ch),dam, PERS(vict, ch));
    stc(log_buf, ch);
    stc("\n\r", ch);
 }
}
...
int count_attacks(CHAR_DATA *ch, CHAR_DATA *vict, int damtype)
{
 ...
 if (IS_CLASS(ch, CLASS_BERSERK))
    count += (ch->bloodlust > 80 ? 80 : ch->bloodlust) / 8;
