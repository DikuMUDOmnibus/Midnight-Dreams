 ...
 * (c) 1999 Listy: Implementor of God Wars Rogue Edition            *
 * Jedi, Lightsabre are trademarks of Lucasfilm                     *
 ********************************************************************/
...
void do_jediskill( CHAR_DATA *ch, char *argument )
{
    ...
    {
		sprintf(buf,"#3Body(%d) Mind(%d) Spirit(%d) Technique(%d).#n\n\r",
		...
		if ( IS_SET(ch->act, PLR_HOLYLIGHT) || IS_AFFECTED(ch,PLR_SHADOWSIGHT) || (IS_SET(ch->special, SPC_JEDI_LEVITATE)))
		{
			...
			if (IS_AFFECTED(ch,PLR_SHADOWSIGHT) )
				send_to_char("#2You can use the force to see other planes of existence.#n\n\r", ch);
		}
		if ( IS_AFFECTED(ch, AFF_SHADOWPLANE) )
		{
			send_to_char("#3Active Spirit Powers.#n\n\r", ch);
			if (IS_AFFECTED(ch, AFF_SHADOWPLANE))
				send_to_char("#3You are inside another plane of existence.#n\n\r", ch);
		}
		if (ch->generation == 1)
			send_to_char("#6You are a lord.#n\n\r", ch);
		else if (ch->generation == 2)
			send_to_char("#6You are a master.#n\n\r", ch);
		else if ((ch->generation == 4) && (IS_SET(ch->special, SPC_PRINCE)) )
			send_to_char("#6You are a knight.#n\n\r", ch);
		else if (ch->generation == 4)
			send_to_char("#6You are an adept.#n\n\r",ch);
		else if (ch->generation == 6)
			send_to_char("#6You are an apprentice.#n\n\r", ch);
		else
