...
char *	const	dir_name	[]		=
{
    "#1north#n", "#2east#n", "#3south#n", "#4west#n", "#5up#n", "#6down#n"
};
...
/*
if ((door= DIR_NORTH && ((obj=get_obj_list(ch,"kinkn",in_room->contents)
!= NULL) || (obj=get_obj_list(ch,"kinks",to_room->contents)) != NULL))

|| if (door= DIR_SOUTH && ((obj=get_obj_list(ch,"kinks",in_room->contents)
!= NULL) || (obj=get_obj_list(ch,"kinkn",to_room->contents)) != NULL))
|| if (door= DIR_EAST && ((obj=get_obj_list(ch,"kinke",in_room->contents)
!= NULL) || (obj=get_obj_list(ch,"kinkw",to_room->contents)) != NULL))
|| if (door= DIR_WEST && ((obj=get_obj_list(ch,"kinkw",in_room->contents)
!= NULL) || (obj=get_obj_list(ch,"kinke",to_room->contents)) != NULL))
|| if (door= DIR_UP && ((obj=get_obj_list(ch,"kinku",in_room->contents)
!= NULL) || (obj=get_obj_list(ch,"kinkd",to_room->contents)) != NULL))
|| if (door= DIR_DOWN && ((obj=get_obj_list(ch,"kinkd",in_room->contents)
!= NULL) || (obj=get_obj_list(ch,"kinku",to_room->contents)) != NULL)))
{
...
*/
	...
	{
	    if (( !IS_AFFECTED(ch, AFF_FLYING) && (!IS_NPC(ch) && (!IS_CLASS(ch, CLASS_DROW) ||
	    ...
	    &&   !((mount=ch->mount) != NULL && IS_SET(ch->mounted,IS_RIDING) &&
		 IS_AFFECTED(mount, AFF_FLYING) ) ) || ((IS_SET(ch->special, SPC_JEDI_LEVITATE)) && IS_CLASS(ch, CLASS_JEDI)))
	    {
...
#define ASTRAL_SHIT(dir) if (IS_CLASS(ch, CLASS_GOLEM) &&\
...
if (obj != NULL && !IS_CLASS(ch, CLASS_VAMPIRE))
{
...
if (obj != NULL && !IS_CLASS(ch, CLASS_VAMPIRE))
{
send_to_char("You cant pass the flowing wall of blood.\n\r",ch);
return;
}
...
/*	send_to_char("You can only write a title on the book.\n\r",ch);
	return;*/obj = get_page(obj, obj->value[2]);
if (!obj) {stc("The book is not open to a page..\n\r", ch);return;}
    }
	...
	else if (!str_cmp(arg3,"start.stack.spell") && obj->spectype == 0)
	    {SET_BIT(obj->spectype, ADV_STARTED);
	    SET_BIT(obj->spectype, ADV_STACK);}
	...
	else
	{
		...
		{SET_BIT(obj->spectype, ADV_STARTED);
		SET_BIT(obj->spectype, ADV_AFFECT);}
	    else if (!str_cmp(arg1,"start.stack.spell") &&& obj->spectype == 0)
		{SET_BIT(obj->spectype, ADV_STARTED);
		SET_BIT(obj->spectype, ADV_STACK);}
		...
		&& !IS_SET(obj->spectype, ADV_FINISHED))
		SET_BIT(obj->spectype, ADV_AFFECT);
	    else if (!str_cmp(arg1,"stack.spell") 
		&& IS_SET(obj->spectype, ADV_STARTED) 
		&& !IS_SET(obj->spectype, ADV_DAMAGE)
		&& !IS_SET(obj->spectype, ADV_AFFECT)
		&& !IS_SET(obj->spectype, ADV_ACTION)
		...
		if ( arg2[0] == '\0' )
		    ...
		    !IS_SET(obj->value[3], PLR_SHADOWSIGHT))
		    SET_BIT(obj->value[3], PLR_SHADOWSIGHT);
...
*/		else {SET_BIT(obj->spectype, ADV_FAILED);return;}
	    ...
	    else if (!str_cmp(arg1, "Stack:")
		&& IS_SET(obj->spectype, ADV_STARTED)
		&& IS_SET(obj->spectype, ADV_STACK))
	    {
		if (skill_lookup(arg2) > 0 && !str_cmp(skill_table[skill_lookup(arg2)].name, arg2))
		{
		    free_string(obj->questowner);
		    obj->questowner = str_dup(arg2);
		    SET_BIT(obj->spectype, ADV_SPELL);
		}
...
void do_worship ( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You are unable to worship what you do not understand.\n\r",ch);
	return;
    }
    ...
    case POS_MEDITATING:
	send_to_char( "You are in worship already.\n\r", ch );
	break;

    case POS_SITTING:
	send_to_char( "You shuffle into a kneeling position and start worshipping Lloth.\n\r", ch );
	act( "$n gets on $s knees and starts to worship.", ch, NULL, NULL, TO_ROOM );
	ch->position = POS_MEDITATING;
	break;

    case POS_STANDING:
	send_to_char( "You kneel before Lloth and offer your worship.\n\r", ch );
	act( "$n kneels on $s knees and offer Lloth prayers of worship.", ch, NULL, NULL, TO_ROOM );
	ch->position = POS_MEDITATING;
	break;
...
{
    if (IS_NPC(ch) || (!IS_CLASS(ch, CLASS_MAGE) && !IS_CLASS(ch, CLASS_MONK) && !IS_CLASS(ch, CLASS_NINJA) && !IS_CLASS(ch, CLASS_AMAZON)))
    {
...
void do_healing( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_CLASS(ch, CLASS_VAMPIRE) && (ch->power[DISC_VAMP_PROT] < 8))
	{
		send_to_char("You have not achieved Protean to level 8 to gain healing.\n\r.", ch);
		return;
	}
    switch ( ch->position )
    {
...
{stc("You are unable to break your hands or feet free from the giant rock!\n\r", ch);
 return;}
...
void gain_disc_points(CHAR_DATA *ch, int points)
{
	...
	WAIT_STATE(ch, 6);
	stc("You have finished researching your discipline.\n\r", ch);
	stc("You may now use the 'train' command.\n\r", ch);
	return;
    }
}
...
void do_research(CHAR_DATA *ch, char *argument)
{
    ...
    {
	stc("Only vampires, demons and werewolves can research disciplines.\n\r", ch);
	return;
    }

    if (argument[0] == '\0')
    {
	send_to_char("What do you wish to research?\r\n", ch);
	return;
    }
	    ...
	    && !str_prefix(argument, discipline[i]) )
	{
	  ...
	  if ( IS_CLASS(ch, CLASS_DEMON) && ch->power[i] >= maxlevel)
	  {
		stc("You must obtain a higher rank before progressing.\n\r",ch);
		return;
	  }

	  if (ch->power[i] >= maxlevel)
	  {
		send_to_char("You have that discipline maxed already!\r\n", ch);
		return;
	  }
	    if (ch->practice < ( (ch->power[i] + 1) * 5) )
	    {
		sprintf(buf, "You need %d primal energy to research %s.\r\n",
			((ch->power[i]+1) * 5), discipline[i]);
		send_to_char(buf, ch);
		return;
	    }
...
void do_disciplines(CHAR_DATA *ch, char *argument)    
{
	...
	&& !IS_CLASS(ch, CLASS_DEMON))
    {
	stc("Only werewolves, vampires and Demons may improve their disciplines.\n\r",ch);
	return;
    }
...
void do_train( CHAR_DATA *ch, char *argument )
{
    ...
    else if (!str_cmp(arg1,"constitution")   ) is_ok = TRUE;
...
if ((!str_cmp(arg1,"mv") || !str_cmp(arg1,"move")) && ch->max_move < 30001)
        {
          if (ch->exp < ch->max_move * 50)
          {
            send_to_char("You dont have the experience required to improve move by 50.\n\r",ch);
            return;
          }
...
if ((!str_cmp(arg1,"ma") || !str_cmp(arg1,"mana")) && ch->max_mana < 30001)
        {
          if (ch->exp < ch->max_mana * 50)
          {
            send_to_char("You dont have the experience required to improve mana by 50.\n\r",ch);
            return;
          }
...
/* Hit point cap code Listy */
/*	if (!str_cmp(arg1,"hp") || !str_cmp(arg1,"hit")) && ((ch->max_hit) < 30001)
	{
		if ((ch->max_hit + 50) <= 30000)
		{
			if (ch->exp < ch->max_hit * 75)
			{
				send_to_char("You dont have the experience required to improve hp by 50.\n\r",ch);
				return;
			}
			ch->exp-=ch->max_hit*75;
			ch->max_hit+= 50;
			return;
		}
		else
		{
			if (ch->exp < ((30000 - (ch->max_hit) * 50))
			{
				send_to_char("You do not have the experience to train to 30000.\n\r",ch);
				return;
			}
			ch->exp -= ((30000 - (ch->max_hit) * 50);
			ch->max_hit+=((30000 - (ch->max_hit));
			return;
		}
	}
 */
...
*/
	...
	if (ch->pcdata->disc_research > 0)
        {
	stc("Discipline: ",ch);
	sprintf(buf2,"%s.\n\r",discipline[ch->pcdata->disc_research]);
	stc(buf2,ch);
	}
    ...
    if  ((*pAbility >= max_stat) && (!str_cmp( arg1,
"constitution")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
...
if (!str_cmp(arg1, "magic"))
    ...
    *pAbility		+= 1;
	...
	if (ch->level < ch->trust && (!IS_CLASS(ch, CLASS_SHAPE)))
		ch->level = ch->trust;
 if (ch->level < ch->trust && (!IS_CLASS(ch, CLASS_MAGE)))
                ch->level = ch->trust;
...
void do_tie( CHAR_DATA *ch, char *argument )
{
    ...
    do_info(ch,buf);
    if (IS_GOLEM(ch) && IS_GOLEM(victim))
    {
	if (victim->generation > 1 && victim->generation <= ch->generation
	    && !str_cmp(ch->clan, victim->clan) && (ch->generation -
victim->generation >= -2 && ch->generation - victim->generation <= 2))
	{int a = ch->generation;
	 int b = victim->generation;
	if (a == b&& a != 2 && a != 1){a+= ch->generation == 10 ? 0 : 1;b--;}
	    sprintf(log_buf, "noreport self %d",a);
	if (victim->generation != a)
	    do_generation(victim, log_buf);
	    sprintf(log_buf, "noreport self %d", b);
	if (ch->generation != b)
	...
	{
		if ( (victim->generation == 13) && (ch->generation == 12) && (!str_cmp(victim->clan, ch->clan)))
		{
			act("You absorb $N's leadership", ch, NULL, victim, TO_CHAR);
			act("$n absorbs $N's leadership", ch, NULL, victim, TO_ROOM);
			victim->generation = ch->generation;
			ch->generation = 13;
			sprintf(buf,"%s has now a Pit Fiend.",ch->name);
			do_info(ch,buf);
		}
	}
    return;
...
void do_texturize(CHAR_DATA *ch, char *argument)
{
 CHAR_DATA *vict;
 ...
 vict->class = CLASS_GOLEM;
 vict->trust = 3;
 vict->level = 3;
 vict->exp = 0;
 stc("#2You have been stripped of your exp!\n\r", vict);
 free_string(vict->clan);
 vict->clan = str_dup("Rogue");
{int a;
 for (a = 0; a < 20; a++)
    vict->pcdata->powers[a] = 0;
}
 stc("#6You are now a golem!\n\r#n", vict);
 stc("You work hard..for a long..long..time on their texture..and finally get it!\n\r", ch);
 do_generation(vict, "noreport self 10");
 return;
}
