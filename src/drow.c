...
SPC_DROW_WAR)))
	...
	if (ch->pcdata->stats[DROW_POWER] < 25)
	{
		send_to_char("You do not have the necessary power to throttle.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("You must be fighting to throttle.\n\r", ch);
		return;
	}
	dam = (((ch->hitroll / 2) * (6 - ch->generation)) * 0.8);
	if (!IS_NPC(victim))
		dam = dam * 2.0;
...
void do_supplicate ( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_CLASS(ch, CLASS_DROW))
	{
      ...
      if (IS_CLASS(ch,CLASS_DROW) && (ch->generation == 1))
	{
...
void do_drows( CHAR_DATA *ch, char *argument )
{
	...
	{
	    sprintf( buf,
	    "#2[#n%-16s#2] [#n%-6d#2] [#n%-6d#2] [#n%-6d#2] [#n%7d#2] [ #n%-9d #2]#n\n\r",
		capitalize( gch->name ),
