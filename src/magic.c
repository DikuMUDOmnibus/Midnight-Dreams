...
void improve_spl( CHAR_DATA *ch, int dtype, int sn )
{
	...
	!IS_CLASS(ch, CLASS_SHAPE) &&
	!(IS_CLASS(ch, CLASS_DROW) && IS_SET(ch->special, SPC_DROW_MAG) && (dtype == 0 || dtype == 1)) &&
	!(IS_CLASS(ch, CLASS_DROW) && IS_SET(ch->special, SPC_DROW_CLE) && (dtype == 2 || dtype == 3)))
	{
		ch->spl[dtype] = 200;
		return;
	}
    if (ch->spl[dtype] >= 275 && !IS_CLASS(ch, CLASS_SHAPE) &&
!IS_CLASS(ch, CLASS_MAGE))
	{
		ch->spl[dtype] = 275;
		return;
	}
    if (ch->spl[1] >= 300 && IS_CLASS(ch, CLASS_VAMPIRE))
         {
               ch->spl[1] = 300;
               return;
         }
    if ((IS_CLASS(ch, CLASS_SHAPE) && ch->spl[dtype] > 295) ||
(IS_CLASS(ch, CLASS_MAGE) && ch->spl[dtype] > 295))
	{
		ch->spl[dtype] = 295;
		return;
	}
    if ((dice1 > ch->spl[dtype] || dice2 > ch->spl[dtype]) || (dice1== 100 || dice2== 100))
		ch->spl[dtype] += 1;
         ...
         if (ch->spl[dtype] == 5  ) sprintf(bufskill,"an apprentice of");
    else if (ch->spl[dtype] == 25 ) sprintf(bufskill,"a student at");
    else if (ch->spl[dtype] == 50 ) sprintf(bufskill,"a scholar at");
    else if (ch->spl[dtype] == 75 ) sprintf(bufskill,"a magus at");
    else if (ch->spl[dtype] == 105) sprintf(bufskill,"an adept at");
    else if (ch->spl[dtype] == 125) sprintf(bufskill,"a mage at");
    else if (ch->spl[dtype] == 155) sprintf(bufskill,"a warlock at");
    else if (ch->spl[dtype] == 175) sprintf(bufskill,"a master wizard at");
    ...
    else if (ch->spl[dtype] == 275) sprintf(bufskill,"a #1r#2o#3g#4u#5e #1a#2r#3c#4h#5m#1a#2g#3e #5of");
    else return;
...
bool is_obj( CHAR_DATA *ch )
{
    ...
    {
	send_to_char("You haven't acheived a high enough Mastery of Morphosis to use spells in Object Form.\n\r",ch);
	return FALSE;
    }
    return TRUE;
}
...
/*
 * Lookup a shape-skill by name.
 */
int shape_lookup( const char *name )
{
    int sn;
	...
	if ( shape_skill[sn].name == NULL )
	    break;
	if ( LOWER(name[0]) == LOWER(shape_skill[sn].name[0])
	&&   !str_prefix( name, shape_skill[sn].name ) )
	    return sn;
    }
...
/*  CHAR_DATA *rch; */
    ...
    if ( skill_table[sn].target == 0 )
    {
    	...
    	ADD_COLOUR(ch,colour,L_MAGENTA);
...
void do_cast( CHAR_DATA *ch, char *argument )
{
   ...
   /*
	...
	/* Spider Form spell fix for Drow Listy */
/*	if (IS_CLASS(ch, CLASS_DROW) && (ch->cur_form == FRM_DROWSPIDER))
	{
		if (!is_obj(ch) )
		{
			send_to_char( "You cannot cast spells in spiderform.\n\r", ch);
			return;
		}
	}
*//*
...
*/
    ...
    {
	if (sn >= 0 && skill_table[sn].skill_level == 65 && IS_CLASS(ch, CLASS_MECHA)) ; else {
	send_to_char( "You can't do that.\n\r", ch );
	return;}
    }/*thats no fun!
    if (skill_table[sn].spell_fun == spell_null)
	{
	send_to_char("That's no spell!\n\r", ch);
	return;
	}*/
	if ( (skill_table[sn].spell_fun == spell_energyflux) && (!IS_CLASS(ch, CLASS_MAGE) && !IS_CLASS(ch, CLASS_SHAPE)) )
	{
		send_to_char("You are unlearned in the disciplines of the mage to cast this.\n\r", ch);
		return;
	}
	if ( (skill_table[sn].spell_fun == spell_major_creation) && (!IS_CLASS(ch, CLASS_MAGE) ))
	{if (!IS_IMMORTAL(ch)) {
		send_to_char("You are unlearned in the disciplines of the mage to cast this.\n\r", ch);
		return;}
	}/*
	if (skill_table[sn].spell_fun == spell_make_bag) 
	{
		send_to_char("That is no longer a spell.\n\r", ch);
		return;
	}*/

	/*    if ( skill_table[sn].sphere == SPHERE_NULL )
    {
	...
	/* mortals healing morted avatars fix */
	if (!IS_NPC(victim) && (ch->level < 3) && ((victim->position == POS_STUNNED) || (victim->position == POS_INCAP)) && (victim->level > 2))
	{
	...
	WAIT_STATE(ch, skill_table[sn].beats / 10);
    else if (skill_table[sn].spell_fun == spell_teleport)
	WAIT_STATE(ch, skill_table[sn].beats / 10);
    else if (skill_table[sn].spell_fun == spell_cure_light)
	WAIT_STATE(ch, skill_table[sn].beats / 10);
    else WAIT_STATE( ch, skill_table[sn].beats );
    improveskill = 0;  
...
void spell_acid_blast( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (IS_ITEMAFF(victim, ITEMA_ACIDSHIELD) && !IS_CLASS(ch, CLASS_MAGE))
return;
    dam = dice( level, 6 );
    hp = victim->hit;
...
void spell_llothbless( int sn, int level, CHAR_DATA *ch, void *vo )
{
   ...
   /* af.location  = APPLY_DAMROLL;
    af.modifier  = 300;
    affect_to_char( victim, &af );*/
    act( "$n Slices his wrists and sacrifices his blood to lloth for Blessing.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You Slice yor wrist letting the blood run free and pour it upon your whip, sacrificing the liquid for a gift.\n\r", victim );
    return;
}
/* SMOO AND JACE DONT TOUCH THIS I KNOW WHAT THE FUCK IM DOING,  If i here
complaints of overrunning ill lower it */

 void spell_amazonbless( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    af.modifier  = 400;
    affect_to_char( victim, &af );
    act( "$n throws her Spear down on The Ground And pray For Artemis's Good word.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You Throw down your spear and Pray to Artemis for a gift.\n\r", victim );
    return;
}
...
void spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        send_to_char( "Go pick on someone your own squirty size!\n\r", ch
);
return;
    }
...
*/
    ...
    paf->modifier	= sn == skill_lookup("enchant weapon") ? level / 5 : 3;
    paf->bitvector	= 0;
...
*/
    ...
    paf->modifier	= sn == skill_lookup("enchant weapon") ? level / 10 : 3;
    paf->bitvector	= 0;
    paf->next		= obj->affected;
    obj->affected	= paf;
...
void spell_enchant_armor( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	{
	send_to_char("Invokers only!\n",ch);
	return;
	}

    if ( obj->item_type != ITEM_ARMOR
    ||   IS_SET(obj->quest, ARMOR_ENCHANTED)
    ||   IS_SET(obj->quest, QUEST_ARTIFACT)
    ||   obj->chobj != NULL )
    {
	send_to_char("You are unable to enchant that piece of equipment\n\r", ch);
	return;
    }
...
*/
    ...
    paf->duration	= -1;
    paf->location	= APPLY_HITROLL;
    paf->modifier	= sn == skill_lookup("enchant armor") ? level / 5
: 1;
    paf->bitvector	= 0;
    paf->next		= obj->affected;
    obj->affected	= paf;
...
*/
    ...
    paf->duration	= -1;
    paf->location	= APPLY_DAMROLL;
    paf->modifier	= sn == skill_lookup("enchant armor") ? level / 8
: 1;
    paf->bitvector	= 0;
    paf->next		= obj->affected;
    obj->affected	= paf;
    ...
    {
	SET_BIT(obj->extra_flags, ITEM_ANTI_EVIL);
	SET_BIT(obj->quest, ARMOR_ENCHANTED);
	act( "$p glows a kinky color.", ch, obj, NULL, TO_CHAR );
	act( "$p glows a kinky color.", ch, obj, NULL, TO_ROOM );
    }
    else if ( IS_EVIL(ch) )
    {
	SET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
...
void spell_fireball( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (IS_ITEMAFF(victim, ITEMA_FIRESHIELD) && !IS_CLASS(ch, CLASS_MAGE))
return;

    level	= UMIN(level, sizeof(dam_each)/sizeof(dam_each[0]) - 1);
    level	= UMAX(0, level);
    dam		= number_range( dam_each[level] / 3.4, dam_each[level] );
    if ( saves_spell( level, victim ) )
	dam /= 2;
if (IS_CLASS(ch, CLASS_MAGE)) dam *= 2;
...
void spell_faerie_fog( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
		if (IS_CLASS(ich, CLASS_MONK) && IS_SET(ich->act, PLR_SACREDINVIS))
		{
			REMOVE_BIT(ich->act, PLR_SACREDINVIS);
			send_to_char( "You emerge from your cloak of invisibility.\n\r", ich );
			act("$n slowly emerges from a cloak of holy invisibility.",ich,NULL,NULL,TO_ROOM);
		}

	/*	if ((IS_CLASS(ich, CLASS_DROW) && (IS_SET(ich->newbits, NEW_DARKNESS)) ) )
		{
			send_to_char("You banish your globe of darkness.\n\r", ich );
			act("The globe of darkness around $n disappears.",ich,NULL,NULL,TO_ROOM);
		...
		if (IS_SET(ch->in_room->room_flags, ROOM_TOTAL_DARKNESS))
		{
			...
			send_to_char("You dispel the darkness.\n\r", ch);

		} */
	    if ( IS_SET(ich->act, PLR_SACREDINVIS)  && IS_CLASS(ich, CLASS_NINJA) )
	    {

			REMOVE_BIT(ich->act, PLR_SACREDINVIS);
			act( "$n appears from the shadows.", ich, NULL, NULL, TO_ROOM );
...
if (IS_CLASS(ch, CLASS_MAGE)) victim->hit = UMIN(victim->hit + 1200, victim->max_hit );
...
void spell_lightning_bolt( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (IS_ITEMAFF(victim, ITEMA_SHOCKSHIELD) && !IS_CLASS(ch,
CLASS_MAGE)) return;

    level	= UMIN(level, sizeof(dam_each)/sizeof(dam_each[0]) - 1);
    level	= UMAX(0, level);
    dam		= number_range( dam_each[level] / 2, dam_each[level] * 2 );
if (IS_CLASS(ch, CLASS_MAGE)) dam *= 3.53;   
    hp = victim->hit;
    damage( ch, victim, dam, sn );
    hp = victim->hit;
    return;
...
void spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    hp = victim->hit;
if (IS_NPC(victim)) dam *= .25;
if (dam >= 2100) dam = number_range(2000, 2500);
    damage( ch, victim, dam, sn );
    return;
}
...
dam *= 1.3;
    hp = victim->hit;
if (IS_NPC(victim)) dam *= .28;
if (dam >= 2000) dam = number_range(2000, 2100);
if (IS_CLASS(ch, CLASS_VAMPIRE)) dam *= 1.5;
    	damage( ch, victim, dam, sn );
    if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_HEAT) )
	dam /= 2;
    hp = victim->hit;
...
void spell_frost_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    hp = victim->hit;
if (IS_NPC(victim)) dam *= .27;
if (dam >= 2150) dam = number_range(2000, 2900);
    damage( ch, victim, dam, sn );
    hp = victim->hit;
    return;
}
...
void spell_gas_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	vch_next = vch->next_in_room;
	if (vch->trust >= 7 && !IS_NPC(vch)) continue;
...
if (IS_NPC(victim)) dam *= .26;
if (dam >= 2100) dam = number_range(2000, 2400);
    damage( ch, victim, dam, sn );
    hp = victim->hit;
    return;
}
...
void spell_portal( int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    ||   victim->in_room == NULL
    ||   IS_NPC(ch)
    ||   (IS_NPC(victim) && !IS_CLASS(ch,CLASS_MAGE))
    ||   (!IS_NPC(victim) && !IS_IMMUNE(victim, IMM_SUMMON) )
...
void spell_energyflux( int sn, int level, CHAR_DATA *ch, void *vo)
{
	...
	victim->mana = UMIN( victim->mana + 75, victim->max_mana);
	update_pos(victim);
...
void spell_transport( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (obj->points > 0)
	{
		send_to_char("You may not transport such a precious item.\n\r", ch);
		return;
	}
...
void spell_chaos_blast( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    dam = dice( level, 23 );
    if ( saves_spell( level, victim ) ) dam *= 1.3;
    damage( ch, victim, dam, sn );
    return;
}
...
void spell_polymorph( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    else if ( !str_cmp( target_name, "wyvern" ) && (IS_SET(ch->extra,
EXTRA_ILLUS)))
{
	int timeamnt = (number_range(30,50));
        if (!IS_NPC(ch) && ch->stance[0] != -1) do_stance(ch,"");
        if (ch->mounted == IS_RIDING) do_dismount(ch,"");
        act("$n polymorphs into a wyvern!",ch,NULL,NULL,TO_ROOM);
        send_to_char("You polymorph into a wyvern!\n\r",ch);
        clear_stats(ch);
        af.type      = sn;
        af.duration  = timeamnt;
        af.location  = APPLY_AC;
        af.modifier  = -1200;
        if (IS_AFFECTED(ch, AFF_FLYING)) af.bitvector = AFF_POLYMORPH;
        else af.bitvector = AFF_POLYMORPH + AFF_FLYING;
        ...
        affect_to_char( ch, &af ); 
        af.location  = APPLY_POLY;
        af.modifier  = POLY_WYVERN;
        affect_to_char( ch, &af );
        sprintf(buf,"%s the wyvern",ch->name);
        free_string(ch->morph);
        ch->morph = str_dup(buf);
        return;
    }
	else if ( !str_cmp( target_name, "serpent" ) && (IS_SET(ch->extra,
	EXTRA_ILLUS)))
    {
        int timeamnt = (number_range(30,50));
        if (!IS_NPC(ch) && ch->stance[0] != -1) do_stance(ch,"");
        if (ch->mounted == IS_RIDING) do_dismount(ch,"");
        act("$n polymorphs into a huge serpent!",ch,NULL,NULL,TO_ROOM);
        send_to_char("You polymorph into a huge serpent!\n\r",ch);
        clear_stats(ch);
        af.type      = sn;
        af.duration  = timeamnt;
        af.location  = APPLY_AC;
    ...
    send_to_char( "You can polymorph into a frog, a fish, a raven, a
wyvern, a serpent, or a (undecided).\n\r",ch);
    return;
}
/*
...
void spell_haste( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !IS_CLASS(ch, CLASS_MAGE))
    {
	send_to_char( "You cannot cast that!.\n\r", ch );
	return;
    }
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    act( "You suddenly feel faster as $n chants archaic words.", ch, NULL,
victim, TO_VICT );
    if ( ch != victim )
	send_to_char( "Ok.\n\r", ch );
    return;
}
void spell_clusterbomb( int sn, int race, CHAR_DATA *ch,void *vo)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    static const sh_int dam_each[] = 
    {
	 0,
	 400,  400,  500,  500,  600,	 700,  700,  700,  700,  700,
	 800,  800,  800,  800,  800,	 900,  900,  900,  900,  900,
	 1000,  1000,  1000,  1000,  1000,	1100, 1100, 1100, 1100, 1100,
	1200, 1200, 1200, 1200, 1200,	1300, 1300, 1300, 1300, 1300,
	1400, 1400, 1400, 1400, 1400,	1500, 1500, 1500, 1500, 1500
    };
    int dam,i;

    race	= UMIN(race, sizeof(dam_each)/sizeof(dam_each[0]) - 1);
    race	= UMAX(0, race);
/* 1 Missile for every 4 status points of the pilot */

    if(ch->race<4) i=1 ;else i=ch->race/4;
    
    for( ; i>0; i--) {
     dam= number_range( dam_each[race] / 2, dam_each[race] * 2 );
	damage(ch, victim, dam, sn);
