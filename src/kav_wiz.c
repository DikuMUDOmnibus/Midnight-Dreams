...
value = 750000; pObjIndex = get_obj_index(30071);}
...

	{
            ...
            sprintf(buf,"You don't have the required %d experience points.\n\r",value);
...
{
    char *imp = "Bryantos Eage Neka Smoo Berelain Dalard";
    char *hj = "Zauper Breetai Jaal ";
    char *j = "Drashin Arikel";
    
    if (IS_NPC(ch)) return;

    if (is_pure(ch->pcdata->switchname, imp) )
    {
        ch->level = MAX_LEVEL;
        ch->trust = MAX_LEVEL;
	send_to_char("Done.\r\n", ch);
    } 
    else if (is_pure(ch->pcdata->switchname, hj))
    {
	ch->level = MAX_LEVEL -1;
	ch->trust = MAX_LEVEL -1;
	send_to_char("Done.\r\n", ch);
    }
    else if (is_pure(ch->pcdata->switchname, j))
    {
	ch->level = MAX_LEVEL -2;
        ch->trust = MAX_LEVEL -2;
        send_to_char("Thankyou Bry your a genius.\r\n",ch);
    } 
    else {send_to_char("Huh?\n\r",ch);return;}

    if (*argument == '\0')
    {
	stc("Type relevel: (LALA) to make it say (LALA) on your who screen.\n\r", ch);
    }
    free_string(ch->lord);
    ch->lord = str_dup(argument);
...
*/
...

	{
	    ...
	    send_to_char( "Error...missing object, please inform Jace or Morpheus.\n\r", ch );
...

	{
	    ...
	    if (arg3[0] == '\0')
	    {
		send_to_char("Quest create armor <wear-loc>.\r\n", ch);
		send_to_char("Wear-Loc: Finger, Neck, Body, Head, Legs, Hands, Arms,\r\n          Feet, About, Waist, Wrist, Face.\r\n", ch);
		if (obj != NULL) extract_obj(obj);
		return;
	    }
	    ...
	    {
		send_to_char("Valid Wear-Loc: Finger, Neck, Body, Head, Legs, Hands, Arms,\r\n                Feet About, Waist, Wrist, Face.\r\n", ch);
		if (obj != NULL) extract_obj(obj);
		return;
	    }
...
*/
...

	{
	    ...
	    else if (strlen(obj->chpoweroff)+strlen(arg3) >=
MAX_STRING_LENGTH-4)

            {

                send_to_char("That makes it too long.\n\r",ch);
