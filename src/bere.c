...
sprintf(buf,"Someone has just placed a BOUNTY of #2%d#n quest points on #4%s's#n head may
the hunt be swift and deadly!",
dtok, victim->pcdata->switchname);
...
void do_mudc( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char("#7MudControl: #7",d->character);
...
void do_questinfo( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected == CON_PLAYING )
	{
	    send_to_char("#1Quest#0Info#3 ->#2",d->character);
	    send_to_char( argument, d->character );
	    send_to_char( "\n\r",   d->character );
	}
    }
...
void do_trivia( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected == CON_PLAYING )
	{
	    send_to_char("#2Triva ->#0",d->character);
	    send_to_char( argument, d->character );
	    send_to_char( "\n\r",   d->character );
	}
    }
...
void do_bonus( CHAR_DATA *ch, char *argument)
{
    ...
    sprintf( buf,"You have bonused %s a whopping %d experience
points.\n\r",
    		victim->name, value);
    		send_to_char(buf, ch);
...
/* top border */
center_to_char( "#4+#0-",ch,72-(2*size) );
...
/* middle */
sprintf(buf,"#4%c",UPPER(argument[0]));
