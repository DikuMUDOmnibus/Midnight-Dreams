...
char *	const	where_name	[] =
{
    ...
    "#1[#3Fourth Hand   #1]#n ",
    "#1[#3On Face       #1]#n ",
    "#1[#3Left Scabbard #1]#n ",
    "#1[#3Right Scabbard#1]#n ",
};
...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    buf[0] = '\0';
    if ( IS_SET(obj->quest, QUEST_ARTIFACT)) strcat(buf, "#1(#7Artifact#1)#n " );
    else if ( IS_SET(obj->quest, QUEST_RELIC)) strcat(buf, "#4(#6Relic#4)#n " );
    else if ( obj->points < 750 && obj->points != 0) strcat(buf,"#3(#2Legendary#3)#n " );
    else if ( obj->points < 1250 && obj->points != 0 ) strcat(buf, "#1(#6Mythical#1)#n " );
    else if ( obj->points < 3000 && obj->points !=0) strcat(buf, "#5(#1Priceless#5)#n " );
    if ( IS_OBJ_STAT(obj, ITEM_INVIS)     )   strcat( buf, "#7(#0Invis#7)#n " );
    ...
    if ( IS_AFFECTED(ch, AFF_DETECT_MAGIC)
         && IS_OBJ_STAT(obj, ITEM_MAGIC)  )   strcat( buf, "#4(#5Magical#4)#n " );
...
int char_damroll(CHAR_DATA *ch)
{
    ...
    if (ch->power[DISC_VAMP_POTE] > 0)
        value += ch->power[DISC_VAMP_POTE] * 10;
...
//        value += ch->xhitroll;
  ...
  if (!IS_NPC(ch))
  {
    ...
    if (ch->power[DISC_VAMP_CELE] > 0)
        value += ch->power[DISC_VAMP_CELE] * 15;
...
void do_look( CHAR_DATA *ch, char *argument )
{
    ...
    {
        sprintf(log_buf, "#3Astral Portal room ##%d\n\r", ch->pcdata->powers[2]);
        stc(log_buf, ch);
        sprintf(log_buf, "#5%s#n", get_astral_room(ch));
        stc(log_buf, ch);
        return;
    }
...
void do_promote (CHAR_DATA *ch, char *argument)
{
    ...
    {
	stc("Lemures cannot progress till they have been accepted into a clan.\n\r", ch);
	return;
    }

    if (strlen(ch->clan) < 1)
    {
	stc("You must be in a clan to promote other demons.\n\r", ch);
	return;
    }

    if (str_cmp(ch->clan, victim->clan) )
    {
	stc("They are not in your clan!\r\n", ch);
	return;
    }
    if ( victim == ch )
    {
    ...
    sprintf(buf, "%s has been promoted by %s.", victim->name, ch->name);
    do_info(ch,buf);
...
send_to_char(buf,ch);
sprintf( buf,"Dragon: %3d Tiger: %3d Monkey: %3d Swallow : %3d\n\r", ch->stance[7],ch->stance[8],ch->stance[9],ch->stance[10]);
send_to_char(buf,ch);
send_to_char(
...
ch->generation,bloodpool,age,clan,blood);
send_to_char(buf,ch);
send_to_char(lin,ch);
send_to_char(
"                              Disciplines\n\r",ch);
...
if (IS_MOOGLE(ch))
{sprintf(log_buf,"#2You scream #6'KUPPO!!! KUPPO!!#2' as you notice you have #3%d#2 kupo nuts!\n\r", ch->pcdata->powers[MOOG_NUTS]);
stc(log_buf, ch);}
  if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_BERSERK))
{
sprintf(buf,
"Your Bloodlust is pumping at %d fold.\n\r",
ch->bloodlust );
stc( buf, ch );
}            
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"It is %d o'clock %s, Day of %s, %d%s the Month of %s.\n\r Midnight Dreams started up at %s\rThe system time is %s\r",
...
sprintf(class , "Newbie       ");
	...
	/* Format it up. */
		...
		else if (IS_CLASS(wch, CLASS_MIND))
		{
			strcpy(openb, "-{::"); strcpy(closeb, "::}-");
		}
		else
	...
	"Rogue" : wch->clan, golem_ranks[wch->generation]);
		...
		else if(IS_CLASS(wch, CLASS_MIND) && wch->generation == 1)
		sprintf (kav, " %sElder Mindflayer%s",openb, closeb);
 ...
 //	else if (IS_CLASS(wch, CLASS_HIGHLANDER) && wch->generation == 1)
	//   	sprintf( kav, " %sMaster Sensai%s", openb, closeb );
		...
		else if (IS_CLASS(wch, CLASS_MIND) && wch->clan != NULL)
			{
				if (strlen(wch->clan) < 1)
					sprintf( kav, " %sMindflayer%s" , openb, closeb);
				if (wch->generation == 3)
					...
					sprintf(kav, " %sSlave of %s%s", openb, wch->clan, closeb);
			}
		else if (IS_CLASS(wch, CLASS_MIND) && wch->clan == NULL)
			{
				sprintf(kav, " %sMindflayer%s", openb, closeb);
...
void do_finger( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( fp = fopen( strsave, "r" ) ) != NULL )
    {
		...
		sprintf(log_buf, "They are a %s with %d status.",
	...
	/* Dunno if this will work, hopefully tells the players class. Adreno */
	/* commented out for now, not sure if it needs and argument not not in the header */
/*	switch(victim->class)
	{
		default:
		send_to_char("They are classless.",ch);
		break;
	    case CLASS_WEREWOLF:
		send_to_char("They are a Werewolf",ch);
		break;
	    case CLASS_VAMPIRE:
		send_to_char("They are a Vampire",ch);
		break;
	    case CLASS_DEMON:
		send_to_char("They are a Demon",ch);
		break;
	    case CLASS_HIGHLANDER:
		send_to_char("They are a Highlander",ch);
		break;
	    case CLASS_DROW:
		send_to_char("They are a Drow",ch);
		break;
	    case CLASS_MONK:
		send_to_char("They are a Monk",ch);
		break;
	    case CLASS_NINJA:
		send_to_char("They are a Ninja",ch);
		break;
	    case CLASS_AMAZON:
		send_to_char("They are an Amazon",ch);
		break;
	    case CLASS_JEDI:
		send_to_char("They are a Jedi",ch);
		break;
	   case CLASS_SHAPE:
		send_to_char("They are a ShapeShifter",ch);
		break;
	}*/
/* hopefully the end of this, but will need to add more as new classes
are made. Adreno */
    ...
    {
	send_to_char("No such finger file exists.\r\n", ch);
