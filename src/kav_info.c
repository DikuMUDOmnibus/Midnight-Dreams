...

{
    ...
    else if (victim->stance[stance] <= 249) sprintf(bufskill,"a grand master of");
	else if (victim->stance[stance] >= 250) sprintf(bufskill,"a rogue master of");
    else return;
...

{
    ...
    else if (ch->spl[dtype] >= 275)  sprintf(bufskill,"the rogue archmage of");
...

{
	...
	if (yy > 0 || (dd > 0 && mm >= 8) || IS_EXTRA(ch, EXTRA_LABOUR))
...

{
    ...
    if (!IS_EXTRA(ch, EXTRA_BORN)) yy += 17; /* Starting age for all
players */
