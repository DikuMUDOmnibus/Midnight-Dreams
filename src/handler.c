...
/*	if ( !str_cmp( str, name ) )
	...
	return TRUE; will this work ? - Bob */

    }

}
bool isname( const char *str, char *namelist )

{
	...
	namelist = oneargument( namelist, name );
...
/* same as is_name, except it doesn't use str_prefix */

bool is_pure( const char *str, char *namelist )
...
if (paf->bitvector & AFF_STUNNED)
{
 if (fAdd)
 {
    if (ch->fighting || ch->position == POS_FIGHTING) stop_fighting(ch,TRUE);
    ch->position = POS_STUNNED;
    act("$n falls to the ground, stunned.", ch, 0, 0, TO_ROOM);
 }
 else
 {
   if (ch->position == POS_STUNNED) {ch->position = POS_STANDING;
	act("$n flips back up to his feet.", ch, 0, 0, TO_ROOM);}
 }
}
...

{
  ...
  else if (!IS_SET(ch->flag2, AFF_REDONE))
...

{
    ...
    || (obj->pIndexData->vnum >= 11400 && obj->pIndexData->vnum < 11414 && !IS_CLASS(ch, CLASS_GOLEM)))
	...
	if (IS_SET(obj->spectype, SITEM_DROW) || obj->pIndexData->vnum == 29600 || obj->pIndexData->vnum == 29606)

	{
...

		{
			...
			act( "You are zapped by the drow item $p and drop it.", ch, obj, NULL, TO_CHAR );

			act( "$n is zapped by the drow item $p and drops it.",  ch, obj, NULL, TO_ROOM );
	...
	{
		if ((!IS_NPC(ch) && !IS_CLASS(ch, CLASS_JEDI)) && ((obj->pIndexData->vnum >= 29709) && (obj->pIndexData->vnum <= 29712)) )
		{
			act( "You are zapped by the jedi item $p and drop it.", ch, obj, NULL, TO_CHAR );
			act( "$n is zapped by the jedi item $p and drops it.",  ch, obj, NULL, TO_ROOM );
			obj_from_char( obj );
			obj_to_room( obj, ch->in_room );
			do_autosave( ch, "" );
			return;
	    ...
	    if ((!IS_NPC(ch) && !IS_CLASS(ch, CLASS_NINJA)) && ((obj->pIndexData->vnum >= 29670) && (obj->pIndexData->vnum <= 29673)))

		{

			act( "You are zapped by the ninja item $p and drop it.", ch, obj, NULL, TO_CHAR );

			act( "$n is zapped by the ninja item $p and drops it.",  ch, obj, NULL, TO_ROOM );
...
/* End */
	    ...
	    if ((!IS_NPC(ch) && !IS_CLASS(ch, CLASS_WEREWOLF)) && (((obj->pIndexData->vnum >= 29696) 
		&& (obj->pIndexData->vnum<= 29698)) || (obj->pIndexData->vnum == 29519)) )

		{

			act( "You are zapped by the werewolf item $p and drop it.", ch, obj, NULL, TO_CHAR );

			act( "$n is zapped by the werewolf item $p and drops it.",  ch, obj, NULL, TO_ROOM );
		...
		if (!IS_NPC(ch) && !IS_CLASS(ch, CLASS_VAMPIRE) && !IS_SET(ch->pcdata->stats[UNI_AFF], VAM_MORTAL) 
		&& ((obj->pIndexData->vnum >= 29703) && (obj->pIndexData->vnum <= 29705) ) )
		{
			act ( "You are zapped by the vampire item $p and drop it.", ch, obj, NULL, TO_CHAR);
			act ("$n is zapped by the vampire item $p and drops it.", ch, obj, NULL, TO_ROOM);
			obj_to_room(obj, ch->in_room);
			do_autosave(ch,"");
			return;
		}
	...
	/* Mage eq drop Adreno */
	    if ((!IS_NPC(ch) && !IS_CLASS(ch, CLASS_MAGE)) && ((obj->pIndexData->vnum >= 29699) && (obj->pIndexData->vnum <= 29701)))

		{

			act( "You are zapped by the mage item $p and drop it.", ch, obj, NULL, TO_CHAR );

			act( "$n is zapped by the mage item $p and drops it.",  ch, obj, NULL, TO_ROOM );
...
/* End */
	...
	/* Monk eq drop Adreno */
	    if ((!IS_NPC(ch) && !IS_CLASS(ch, CLASS_MONK)) && ((obj->pIndexData->vnum >= 29706) && (obj->pIndexData->vnum <= 29708)))

		{

			act( "You are zapped by the monk item $p and drop it.", ch, obj, NULL, TO_CHAR );

			act( "$n is zapped by the monk item $p and drops it.",  ch, obj, NULL, TO_ROOM );
...
/* amazon eq drop Adreno  */
	    if ((!IS_NPC(ch) && (!IS_CLASS(ch, CLASS_AMAZON)) && ch->class != 256) &&
((obj->pIndexData->vnum >= 27943) && (obj->pIndexData->vnum <= 27949)))

		{

			act( "You are zapped by the amazon item $p and drop it.", ch, obj, NULL, TO_CHAR );

			act( "$n is zapped by the amazon item $p and drops it.",  ch, obj, NULL, TO_ROOM );
...
/*	    if (((IS_NPC(ch) || !IS_CLASS(ch, CLASS_DEMON)) && IS_SET(obj->spectype, SITEM_DEMONIC)) && !((obj->pIndexData->vnum >= 29670) && (obj->pIndexData->vnum <= 29673)) && !((obj->pIndexData->vnum >= 29699) && (obj->pIndexData->vnum <= 29701)) && !((obj
->pIndexData->vnum >= 29706) && (obj->pIndexData->vnum <= 29708)) && !((obj->pIndexData->vnum >= 27943) && (obj->pIndexData->vnum <= 27948)))*/
		if (!IS_NPC(ch) && (!IS_CLASS(ch, CLASS_DEMON)) && ( ((obj->pIndexData->vnum >= 27931) && (obj->pIndexData->vnum <= 27942)) || ((obj->pIndexData->vnum >= 29650) && (obj->pIndexData->vnum <= 29663)) ) )
		{

			act( "You are zapped by the demonic item $p and drop it.", ch, obj, NULL, TO_CHAR );

			act( "$n is zapped by the demonic item $p and drops it.",  ch, obj, NULL, TO_ROOM );
...
/*		if (((IS_NPC(ch) || !IS_IMMORTAL(ch)) && IS_SET(obj->quest,QUEST_RELIC) && !IS_SET(obj->spectype,SITEM_DEMONIC) && !IS_SET(obj->spectype,SITEM_HIGHLANDER) && !IS_CLASS(ch, CLASS_WEREWOLF) &&  obj->chobj == NULL) && !((obj->pIndexData->vnum >= 29670) &
& (obj->pIndexData->vnum <= 29673)) && !((obj->pIndexData->vnum >= 29699) && (obj->pIndexData->vnum <= 29701)) && !((obj->pIndexData->vnum >= 29706) && (obj->pIndexData->vnum <= 29708)) && !((obj->pIndexData->vnum >= 27943) && (obj->pIndexData->vnum <= 27
948)))
 ...
 */
    ...
    if (((IS_NPC(ch) || !IS_CLASS(ch, CLASS_HIGHLANDER)) && IS_SET(obj->spectype, SITEM_HIGHLANDER)) && !((obj->pIndexData->vnum >= 29670) && (obj->pIndexData->vnum <= 29673)) && !((obj->pIndexData->vnum >= 29699) && (obj->pIndexData->vnum <= 29701)) && !
((obj->pIndexData->vnum >= 29706) && (obj->pIndexData->vnum <= 29708)))

    {

	act( "You are zapped by the highlander's item $p and drop it.", ch, obj, NULL, TO_CHAR );

	act( "$n is zapped by the highlander's item $p and drops it.",  ch, obj, NULL, TO_ROOM );
	...
	if (sn == 19 && !IS_ITEMAFF(ch, ITEMA_PEACE))
	{
	    SET_BIT(ch->itemaffect, ITEMA_PEACE);
	    act("$n fades into the peaceful-realm", ch, 0, 0, TO_ROOM);
	    stc("You are now immune from pk!\n\r", ch);
	}
...
void char_to_jelly( CHAR_DATA *ch, CHAR_DATA *gummy)
{
 char_from_room(ch);
 char_to_room(ch,get_room_index(9));
 gummy->jellybelly2 = ch;
 return;
}

void obj_to_jelly( OBJ_DATA *obj, CHAR_DATA *gummy)
{
 if (obj->in_room) obj_from_room(obj);
 if (obj->carried_by) obj_from_char(obj);

 obj->next_content = gummy->jellybelly1;
 gummy->jellybelly1 = obj;
 return;
}

void obj_from_jelly( OBJ_DATA *obj, CHAR_DATA *gummy)
{
 OBJ_DATA *kink;
   if (gummy->jellybelly1 == obj)
	gummy->jellybelly1 = gummy->jellybelly1->next_content;
   else
   {
	for (kink = gummy->jellybelly1; kink; kink = kink->next_content)
	{
	    if (kink && kink->next_content == obj) break;
	}
	if (!kink || kink->next_content == NULL)
	{
	    bug("obj_from_jelly..obj isn't in belly",0);
	    return;
	}
	kink->next_content = obj->next_content;
    }
...
void char_from_jelly( CHAR_DATA *ch, CHAR_DATA *gummy)
{
 CHAR_DATA *kink;
   if (gummy->jellybelly2 == ch)
	gummy->jellybelly2 = gummy->jellybelly2->next_in_room;
   else
   {
	for (kink = gummy->jellybelly2; kink; kink = kink->next_in_room)
	{
	    if (kink && kink->next_in_room == ch) break;
	}
	if (!kink || kink->next_in_room == NULL)
	{
	    bug("char_from_jelly..ch isn't in belly",0);
	    return;
	}
	kink->next_in_room = ch->next_in_room;
    }
...
{if (IS_NPC(rch) && rch->pIndexData->vnum > 11300 && rch->pIndexData->vnum
< 11399 && !str_cmp(arg,"tty")) ; else
...
if (get_char_room(ch,argument) != NULL) return
get_char_room(ch,argument);
    if ( (number = number_argument(argument, arg)) == 0
...
if (is_pure("holylight",ch->immpower)||inti) return TRUE;

if (!IS_CLASS(ch, CLASS_DEMON) && !IS_CLASS(victim, CLASS_DEMON) &&
(!IS_IMMORTAL(victim)))
if ((ch->generation == 1) && (victim->generation != 1)) return TRUE;

if (IS_CLASS(ch, CLASS_DEMON) && !IS_CLASS(victim, CLASS_DEMON) &&
(!IS_IMMORTAL(victim)))
if ((ch->generation == 15) && (victim->generation != 1)) return TRUE;

if (IS_CLASS(ch, CLASS_DEMON) && IS_CLASS(victim, CLASS_DEMON) &&
(!IS_IMMORTAL(victim)))
if ((ch->generation == 15) && (victim->generation != 15)) return TRUE;

if (!IS_CLASS(ch,CLASS_DEMON) && IS_CLASS(victim,CLASS_DEMON) &&
(!IS_IMMORTAL(victim)))
if ((ch->generation == 1) && (victim->generation != 15)) return TRUE;

if (ch->spec_fun == spec_lookup("spec_eater")) return TRUE;

if (is_affected(ch, skill_lookup("mecha sleep")))
     return FALSE;

if (victim->act & PLR_SACREDINVIS && IS_CLASS(victim,CLASS_SHAPE))
{
    if (IS_CLASS(ch, CLASS_SHAPE)) ;//return TRUE;
    else if (IS_IMMORTAL(ch)) ;//return TRUE;
    else if (ch->mana - 499 > victim->mana) ;//return TRUE;
    else return FALSE;
}

    if ( victim->blinkykill != NULL && !IS_IMMORTAL(ch))
    ...
    &&   get_trust( ch ) < get_trust( victim ) && (!IS_CLASS(victim,
CLASS_AMAZON) && !IS_IMMORTAL(ch)))

	return FALSE;
   if (IS_CLASS(victim, CLASS_AMAZON) && !IS_IMMORTAL(ch) &&
IS_SET(victim->act, PLR_WIZINVIS)) return FALSE;
...
/*        else
    ...
    if ( IS_SET(victim->act, PLR_SACREDINVIS) && !IS_IMMORTAL(ch))

	{

		if (!IS_CLASS(ch, CLASS_MONK) && !IS_CLASS(victim, CLASS_NINJA))

			return FALSE;



		if (IS_CLASS(victim, CLASS_MONK) && IS_CLASS(ch, CLASS_MONK) && ch->pcdata->powers[PMONK] < 3)

			return FALSE;

	

		if (IS_CLASS(victim, CLASS_NINJA) && IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_SORA] < 3)

			return FALSE;



		if (IS_CLASS(victim, CLASS_NINJA) && (victim->clan != ch->clan) && IS_CLASS(ch, CLASS_NINJA))
