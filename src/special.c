...
const   struct  spec_table	spec_table     [50]     = 
{
    ...
    {"spec_cast_judje",		spec_cast_judge},
...
return FALSE;
 ...
 if (state == 19)
 {
    do_say(ch,"#2You will be given around 5 minutes to prepare, then the devil will #1\"Welcome\"#2 you.");
 }
 ...
 {
    world_peace = FALSE;
    do_echo(ch,"#2Jesus wishes all the good in the worlds, good luck, you will need it.");
 }
 return TRUE;
}
...
while (!get_room_index(3000+a)) a = number_range(1,100);
    ...
    act("#2$n's eyes flicker with gummy energy!#n", ch, 0, 0, TO_ROOM);
    act("#2Your eyes flicker with gummy energy!#n", ch, 0, 0, TO_CHAR);
    char_from_room(ch);
    char_to_room(ch, get_room_index(3000+a));
    do_look(ch, "");
    act("#7$n appears in a blast of milk.", ch, 0, 0, TO_ROOM);
 ...
 {
    case 1:
	act("$n dances around in $m jelly jelly way.", 
	    ch,0,0,TO_ROOM);
	...
	break;
    case 3:
	if (!ch->in_room->people) return FALSE;
	for (vch = ch->in_room->people; vch; vch = vch->next_in_room)
	    if (IS_NPC(vch) && vch->spec_fun != ch->spec_fun) break;
	...
	char_from_room(vch);
	char_to_jelly(vch, ch);
	break;
    case 4:
	if (!ch->jellybelly1)
	{
	    if (!get_obj_index(6) || number_range(1,3) == 1) return FALSE;
	...
	act("$n burps up $p.", ch, obj, 0, TO_ROOM);
	act("You burp up $p.", ch, obj, 0, TO_CHAR);
	obj_from_jelly(obj, ch);
	obj_to_room(obj, ch->in_room);
	break;
    case 5:
	if (!ch->jellybelly2)
	{
	    if (!get_mob_index(5) || number_range(1,3) == 1) return FALSE;
