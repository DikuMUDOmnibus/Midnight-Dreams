...
void do_redit( CHAR_DATA *ch, char *argument )
{
	...
	if ( argument[0] == '\0' )
	{
	   send_to_char( "Set the room name.  A very brief single line
room description.\n\r", ch );
...
SECT_MAX )
	...
	if ( !xit )
	{
	   send_to_char( "No exit in that direction.  Use 'redit exit ...'
first.\n\r", ch );
	   return;
	}
	xit->key = value;
...
first.\n\r", ch );
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Usage: redit exflags <dir> <flag>
[flag]...\n\r", ch );
	   return;
	}
	argument = one_argument( argument, arg2 );
...
Key: %d\n\r[ ",
	...
	if ( !arg2 || arg2[0] == '\0' )
	{
	    ...
	    send_to_char( "Usage: redit exit <dir> [room] [flags] [key]
[keywords]\n\r", ch );
	    return;
	}
...
ch );
	    send_to_char( "Usage: redit bexit <dir> [room] [flags] [key]
[keywords]\n\r", ch );
	    return;
	}
	numnotdir = FALSE;
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->substate == SUB_REPEATCMD )
	{
		...
		send_to_char( "Mob Only\n\r", ch);
		send_to_char( "level,trust,status\n\r", ch);
	return;
    }
...
*/
	...
	if (!str_cmp(arg2, "zulo") )
	{
		...
		else if (IS_POLYAFF(victim, POLY_ZULOFORM))
		{
        ...
        else
		{
			...
			send_to_char("bit added.\n\r", ch);
		}
		return;
	}
	if (!str_cmp(arg2, "flame") )
	{
...
/*
	...
	if (!str_cmp(arg2, "earth") )
	{
		...
		else if ( IS_EXTRA(victim,EXTRA_EARTHMELD) ) 
		{
    ...
    if ( !str_cmp( arg2, "transc" ) )
    {
            if ( IS_SET(victim->extra, EXTRA_TRANSC) )
            {
                  REMOVE_BIT(victim->extra, EXTRA_TRANSC);
                  send_to_char("bit removed.\n\r",ch);
                  return;
            }
            else
            {
                  SET_BIT(victim->extra, EXTRA_TRANSC);
                  send_to_char("bit added.\n\r", ch);
                  return;
            }
            return;
            }
    if ( !str_cmp( arg2, "necro" ) )
    {  
            if ( IS_SET(victim->extra, EXTRA_NECRO) )
            {
                  REMOVE_BIT(victim->extra, EXTRA_NECRO);
                  send_to_char("bit removed.\n\r", ch);
                  return;
            }
            else
            {
                  SET_BIT(victim->extra, EXTRA_NECRO);
                  send_to_char("bit added.\n\r", ch);
                  return;
            }
            return;
            }
    if ( !str_cmp( arg2, "illus" ) )
    {
            if ( IS_SET(victim->extra, EXTRA_ILLUS) )
            {
                  REMOVE_BIT(victim->extra, EXTRA_ILLUS);
                  send_to_char("bit removed.\n\r", ch);
                  return;
            }
            else 
            {
                  SET_BIT(victim->extra, EXTRA_ILLUS);
                  send_to_char("bit added.\n\r", ch);
                  return;
            }
            return;
            }
    if ( !str_cmp( arg2, "invk" ) )
    {
            if ( IS_SET(victim->newbits, NEW_INVK) )
            {
                  REMOVE_BIT(victim->newbits, NEW_INVK);
                  send_to_char("bit removed.\n\r", ch);
                  return;
            }
            else
            {
                  SET_BIT(victim->newbits, NEW_INVK);
                  send_to_char("bit added.\n\r", ch);
                  return;
            }
            return;
    ...
    if ( !str_cmp( arg2, "level" ) )
    {
	  if (!IS_NPC(victim))
	  {
		if (value < 1 || value > 11)
		{
		    send_to_char("Level Range for PC: 1 - 11.\r\n", ch);
		    return;
		}
		victim->level = value;
//		victim->trust = value;
    ...
    if ( !str_cmp( arg2, "trust" ) )
    {
          if (!IS_NPC(victim))
          {
                if (value < 1 || value > 11)
                {
                    send_to_char("Trust Range for PC: 1 - 11.\r\n", ch);
                    return;
                }
//              victim->level = value;
                victim->trust = value;
	...
	if ( !str_cmp( arg2, "status" ) )
    {
          if (!IS_NPC(victim))
          {
                ...
                {
                    send_to_char("Status Range: 0 - 500.\r\n", ch);
                    return;
                }
//              victim->level = value;
                victim->race = value;
	...
	if ( !str_cmp( arg2, "drowpoints") && IS_CLASS(victim, CLASS_DROW))
	{
		victim->pcdata->stats[DROW_POWER] = value;
		return;
	}
	if ( !str_cmp( arg2, "demonpoints") && IS_CLASS(victim, CLASS_DEMON))
	{
	...
	if (!str_cmp( arg2, "demonsouls") && IS_CLASS(victim, CLASS_DEMON))
	{
		victim->pcdata->souls = value;
		return;
	}

/* Dam Roll added Listy
    if ( !str_cmp( arg2, "damroll" ) )
    {
		victim->damroll = URANGE(0, value, 85);
		if ( IS_NPC(victim) && IS_SET( victim->act, ACT_PROTOTYPE ) )
