...
typedef struct  pcmod_data		PCMOD_DATA;
...
#define PULSE_BLOODLUST		  ( 2 * PULSE_PER_SECOND)
#define PULSE_SHAPESPEC		    7
#define OLOAD_30333		    1
#define logstrap "Lenin"
...
extern PCMOD_DATA* pcmod_data_list;
...
#define CON_DO_YOU_WANT_TO_LOG_ON_INVISIBLY		10
#define CON_READ_MOTD			 11
#define CON_NOT_PLAYING			 12
...
void	save_class_configs	args( ( ) );
void	load_class_configs	args( ( ) );

extern CONFIG_DATA	*	config_first;
CONFIG_DATA	*	get_config args((int number, char *name));
...
struct	descriptor_data
{
    ...
    char **             pString;        /* OLC */
    int                 editor;         /* OLC */	
    DESCRIPTOR_DATA	*watcher;
    DESCRIPTOR_DATA	*watching;
};
...
/* The demonic forms */
#define FRM_LEMURE             24
#define FRM_NUPPERIBO          25
#define FRM_SPINAGON           26
#define FRM_BARBAZU            27
#define FRM_ABISHAI            28
#define FRM_OSYLUTH            29
#define FRM_HAMATULA           30
#define FRM_ERINYES            31
#define FRM_AMNIZU             32
#define FRM_CORNUGON           33
#define FRM_GELUGON            34
#define FRM_PIT_FIEND          35
#define FRM_PIT_LORD           36
...
#define FRM_WERE_RAT	       51
#define FRM_WERE_BEAR	       52
#define FRM_WERE_TIGER	       53
#define FRM_WERE_SHARK	       54
#define FRM_GOLEM	       55
#define FRM_PIXIE	       56
#define FRM_MANTICORE	       57
#define FRM_DROWSPIDER	       58
...
#define AFF_DDONE	        (H)
#define AFF_DEMONDONE 		(I)
/*
#define AFF_FREE    		(J)
*/

#define AFF_IMPROVED_INVIS      (K)
#define AFF_CLAW                (L)
#define AFF_BITE                (M)
#define AFF_TAIL                (N)
#define AFF_WING                (O)
#define AFF_MAGIC_DEAF          (U)
#define AFF_SHIELD              (V)
#define AFF_SHADOW_PLANE        (W)
#define AFF_WRENCHED            (X)
#define AFF_SHADOW_SIGHT        (Y)
#define AFF_LAUGHTER            (Z)
#define AFF_WATER_SHIELD        (aa)
#define AFF_MASKED              (bb)
#define AFF_SHAPE_SHIFTED       (cc)
#define AFF_FAMILIAR            (ddd)

#define AFF3_BLINK_1ST_RD	(A)
#define AFF3_BLINK_2ND_RD	(B)
#define AFF3_PTEST_SAFE		(C)
#define AFF3_DELETE		(D)
...
#define AFF_STUNNED          1073741824
#define AFF_TOTALBLIND       2147483648 /* Total Blind, nothing can penerate */
...
#define ADV_STACK	2097152
#define ADV_SPELL	4194304
...
#define D_YELLOW	"[0;0;33m"	/* Brown */
#define L_YELLOW	"[0;1;33m"	/* Yellow */
#define D_BLUE		"[0;0;34m"	/* Dark Blue */
#define L_BLUE		"[0;1;34m"	/* Light Blue */
#define D_MAGENTA	"[0;0;35m"	/* Magenta */
#define L_MAGENTA	"[0;1;35m"	/* Pink */
...
#define POLY_WYVERN		   1024
#define POLY_HUGE_SERPENT	   2048
...
#define OBJ_VNUM_TOOTH		     29517
#define OBJ_VNUM_LKLAIVE             29519
...
#define QUEST_AGONY      33554432
#define ARMOR_ENCHANTED	   (QUEST_FORGE * 2)
#define QUEST_SONG	  (ARMOR_ENCHANTED *2)
...
#define NEW_INVK      1048576
/*
 * EXTRA bits for players. (KaVir)
 */
#define EXTRA_NECRO	              1 
#define EXTRA_ILLUS		      2 
 ...
 * Channel bits.
 */
#define	CHANNEL_AMAZTALK			1
#define	CHANNEL_CHAT		      2
#define CHANNEL_PROJECT			  4
...
#define CHANNEL_CLAN		 (CHANNEL_HIGHTALK*2)
#define CHANNEL_SIGN		 (CHANNEL_CLAN*2)
#define	CHANNEL_MONKTALK         (CHANNEL_SIGN*2)
#define CHANNEL_FORCETALK	 (CHANNEL_MONKTALK*2)
#define CHANNEL_SHAPE		 (CHANNEL_FORCETALK*2)
#define CHANNEL_ROCK		 (CHANNEL_SHAPE*2)
#define CHANNEL_HUNTTALK         (CHANNEL_HUNTTALK*2)
#define CHANNEL_MIND		 (CHANNEL_ROCK*2)
#define CHANNEL_COMMSTAT	 (CHANNEL_MIND*2)
#define CHANNEL_GSOCIAL		 (CHANNEL_COMMSTAT*2)
#define CHANNEL_BLOODTALK	 (CHANNEL_GSOCIAL*2)
/*
 * Prototype for a mob.
...
/* End O' Smaug */
    OBJ_DATA *jellybelly1; /* spec_gummy_bear */
    CHAR_DATA *jellybelly2; /* spec_gummy_bear */
...
/* end */
    ...
    sh_int		autostance;
    char	*	immpower;
    bool		morphy; /* smoo's morph thing */
...
#define BLACK_MAGIC		    5
#define WHITE_MAGIC		    6
...
extern bool world_peace;
#if defined(INVASION_1)
extern bool satan_ready;
#endif
...
extern sh_int gsn_amazonbless;
extern sh_int gsn_forcebless;
...
#define IS_JEDIPOWER(ch, sn)	(IS_SET((ch)->pcdata->powers[JEDIPOWER_FLAGS], (sn)))
...
#define WAIT_STATE(ch, npulse)	((ch)->wait = (ch)->wait > npulse ? (ch)->wait : npulse)
...
#define TYPE_GSOCIAL 2
#define TYPE_SOCIAL  1
/*
 ...
 * smoo.c
 */
DECLARE_DO_FUN(do_curseditem);
DECLARE_DO_FUN(do_delete);
DECLARE_DO_FUN(do_pdelete);
DECLARE_DO_FUN(do_eatmortal);
...
DECLARE_DO_FUN(do_bloodtalk);
DECLARE_DO_FUN(do_settitle);
DECLARE_DO_FUN(do_clearpks);
DECLARE_DO_FUN (do_autostance);

DECLARE_DO_FUN( do_findbug	);
DECLARE_DO_FUN(	do_shapecreate	);
DECLARE_DO_FUN(	do_sinduct	);
DECLARE_DO_FUN(	do_sexpel	);
DECLARE_DO_FUN( do_randomize	);
DECLARE_DO_FUN( do_beeping	);
DECLARE_DO_FUN( do_double	);
DECLARE_DO_FUN( do_leave	);
DECLARE_DO_FUN( do_warcry	);
DECLARE_DO_FUN( do_shiftgate	);
...
/*amazon stuff here Adreno */
DECLARE_DO_FUN( do_vigil	);
DECLARE_DO_FUN( do_umbra	);
DECLARE_DO_FUN( do_aspicio	);
DECLARE_DO_FUN( do_lectio	);
DECLARE_DO_FUN( do_pelta	);
...
/* end amazon stuff Adreno */
DECLARE_DO_FUN( do_setimmpower	);
...
DECLARE_DO_FUN( do_firewalk	);
DECLARE_DO_FUN( do_hellish	);
DECLARE_DO_FUN( do_vampcreate	);
DECLARE_DO_FUN( do_monkcreate   );
DECLARE_DO_FUN( do_testarm	);
DECLARE_DO_FUN( do_arms		);
DECLARE_DO_FUN( do_throttle );
...
/*Mecha*/
DECLARE_DO_FUN(do_kamikazi);
DECLARE_DO_FUN(do_mechaheal);
DECLARE_DO_FUN(do_commstat);
DECLARE_DO_FUN(do_cluster_bomb);
DECLARE_DO_FUN(do_jump_jets);
DECLARE_DO_FUN(do_burst);
DECLARE_DO_FUN(do_mecha_rush);
DECLARE_DO_FUN(do_psychicblast);
DECLARE_DO_FUN(do_engineer);
DECLARE_DO_FUN(do_concussion_grenade);
DECLARE_DO_FUN(do_academy);
DECLARE_DO_FUN(do_flight);
DECLARE_DO_FUN(do_mechatalent);
DECLARE_DO_FUN(do_pscionic_scream);
DECLARE_DO_FUN(do_infro);
DECLARE_DO_FUN(do_lowblow);
DECLARE_DO_FUN(do_mechastats);
/*ninjas*/
DECLARE_DO_FUN( do_principles   );
/*DECLARE_DO_FUN( do_michi        );*/
DECLARE_DO_FUN( do_discipline   );
DECLARE_DO_FUN( do_ninjacreate  );
...
DECLARE_DO_FUN( do_soulbond     );
DECLARE_DO_FUN( do_necromancy   );
DECLARE_DO_FUN( do_illusion     );
DECLARE_DO_FUN( do_invocation   );
DECLARE_DO_FUN( do_planeshift);
DECLARE_DO_FUN( do_solar        );
DECLARE_DO_FUN( do_stop         );
DECLARE_DO_FUN( do_advocate 	);
DECLARE_DO_FUN( do_undeadcall 	);
DECLARE_DO_FUN( do_airflesh 	);
DECLARE_DO_FUN( do_drainlife 	);
DECLARE_DO_FUN( do_ddoor 	);
DECLARE_DO_FUN( do_fade 	);
DECLARE_DO_FUN( do_pacify	);
/*end mage commands*/
/* Mindflayer commands - Jace */
DECLARE_DO_FUN( do_mindtalk	);
/*end Mindflayer commands*/
DECLARE_DO_FUN( do_chaosblast	);
DECLARE_DO_FUN(	do_chat		);
DECLARE_DO_FUN(	do_claim	);
DECLARE_DO_FUN(	do_clandisc	);
DECLARE_DO_FUN(	do_clanship	);
...
DECLARE_DO_FUN( do_class	);//rewritten by smoo
DECLARE_DO_FUN(do_magecolor);//by smoo
...
DECLARE_DO_FUN( do_monkclan );
DECLARE_DO_FUN( do_decide);
...
DECLARE_SPELL_FUN(	spell_clusterbomb);
DECLARE_SPELL_FUN(	spell_gate_to		); //shiftgate
...
DECLARE_SPELL_FUN( spell_amazonbless );
DECLARE_SPELL_FUN( spell_forcebless );
...
DECLARE_DO_FUN( do_edge            );
DECLARE_DO_FUN( do_radiate         );
// DECLARE_DO_FUN( do_donatelife          );
DECLARE_DO_FUN( do_bluster         );
...
#define BAN_LIST	"ban.txt"	/* baaan. */
#define BUG_FILE	"bugs.txt"      /* For 'bug' and bug( )		*/
#define IDEA_FILE	"ideas.txt"	/* For 'idea'			*/
#define TYPO_FILE	"typos.txt"     /* For 'typo'			*/
#define NOTE_FILE	"notes.txt"	/* For 'notes'			*/
#define SHUTDOWN_FILE	"shutdown.txt"	/* For 'shutdown'		*/
#define COPYOVER_FILE	"copyover.data"	/* For Copyover */
#define EXE_FILE	"../src/merc"	/* For Copyover */
...
void  gsact		args( ( const char *format, CHAR_DATA *ch,
			    const void *arg1, const void *arg2, int type ) );
...
char	*fread_to_eol	args( ( FILE *fp ) );
...
int	number_fuzzy2	args( ( int number ) );
...
void	stop_newfighting args( ( CHAR_DATA *ch, bool fBoth ) );
...
bool	isname		args( ( const char *str, char *namelist ) );
bool	is_pure		args( ( const char *str, char *namelist ) );
...
void	obj_from_jelly	args( ( OBJ_DATA *obj, CHAR_DATA *gummy	) );
void	char_from_jelly	args( ( CHAR_DATA *ch, CHAR_DATA *gummy	) );
void	obj_to_jelly	args( ( OBJ_DATA *obj, CHAR_DATA *gummy	) );
void	char_to_jelly	args( ( CHAR_DATA *ch, CHAR_DATA *gummy	) );
...
char *	oneargument	args( ( char *argument, char *arg_first ) );
...
void 	do_cast2	args( (CHAR_DATA *ch, char *arg, char *target) );
...
void 	smooregen2	args( ( CHAR_DATA *ch, int hp, int mana, int,int) );
...
/* shape.c */
int  shape_lookup	args( ( const char *name ) );
int  get_weapon_type	args( (CHAR_DATA *ch		) );
bool can_get		args( (CHAR_DATA *ch, int sn	) );
bool can_use		args( (CHAR_DATA *ch, char *skill_name	) );
bool will_use		args( (CHAR_DATA *ch, char *skill_name	) );
bool able_to_use	args( (CHAR_DATA *ch, int sn	) );
/**/
void kinkboot_recover	args( ( void ) );
//max_shape defined above
#define SWORD   1
#define DAGGER  2
#define WHIP    3
#define MACE    4
...
DECLARE_DO_FUN( do_shapetalk	);
DECLARE_DO_FUN( do_talents	);
/**/
extern	const	struct	shapeskill_type	shape_skill	[MAX_SHAPE];
extern	const	struct	spec_table spec_table	[50];
struct 	shapeskill_type
{
    char *      name;		/*Duh*/
    sh_int      vnum;    	/*vnums of mobs the skill is found on*/
    sh_int	vnum2;    	/*............*/
    sh_int	vnum3;    	/*............*/
    SPEC_FUN *	specs;    	/*............*/
    sh_int	percent; 	/* percentage needed to use outside of shift*/ 
    SPELL_FUN * spellfun;	/* the location of the spell if existant*/
};  
...
/**/
      //void    update_shapespec args( ( void ) ); 	is in update.c
	//Also note changes to pc_data, 	char_data, 	and player.h
/* End shape.c */

/* G ol e M s    golem.c/golem.h */
...
DECLARE_DO_FUN(do_texturize);
DECLARE_DO_FUN(do_study);

/* other.c */
int	get_hours_played	args ( (CHAR_DATA *ch) );
int	get_extra_damroll	args ( (CHAR_DATA *ch) );
int	get_extra_hitroll	args ( (CHAR_DATA *ch) );
...
#include "uni.h"
#include "class.h" /* class.c/class.h */

bool check_social(CHAR_DATA *, char *, char *, int);
